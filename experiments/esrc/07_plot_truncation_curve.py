...
def main() -> None:
    ...
    paths = Paths(
        predictions_csv=project_root / "results/tables/pool_en_reason_predictions.csv",
        out_figure_with_ci=project_root / "results/figures/pool_en_accuracy_curve_with_ci.png",
        out_table_csv=project_root / "results/tables/pool_en_accuracy_by_truncation.csv",
    )
        ...
        summary_rows.append(
            {
                ...
                "n": n,
                "correct": correct,
                "accuracy": float(accuracy) if np.isfinite(accuracy) else float("nan"),
    ...
    try:
        ...
        raise RuntimeError(
            "matplotlib is required but not installed. Install it, e.g.\n"
    ...
    # errorbar wants symmetric or (2, N) array; build asymmetric distances
