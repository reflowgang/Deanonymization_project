...
def main() -> None:
    ...
    paths = Paths(
        predictions_csv=project_root / "results/tables/pool_en_reason_predictions_clean.csv",
        curve_csv=project_root / "results/tables/pool_en_precision_recall_curve.csv",
        summary_csv=project_root / "results/tables/pool_en_recall_at_precision.csv",
    )
    ...
    print(
        "T_level\tn_total\tn_correct_total\ttop1_accuracy\t"
        "recall_at_90_precision\trecall_at_99_precision"
    )
    for _, row in summary_out.iterrows():
        print(
            f"{row['T_level']}\t{int(row['n_total'])}\t{int(row['n_correct_total'])}\t"
