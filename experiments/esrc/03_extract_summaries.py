...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Extract summaries from POOL-EN truncated query profiles using a prompt file."
    )
    p.add_argument(
        "--levels",
        nargs="+",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Process only the first N files per level (for testing).",
    )
...
def main() -> None:
    ...
    paths = Paths(
        ...
        summaries_root=project_root / "data/esrc/pool_en/summaries",
        extract_log_csv=project_root / "results/tables/pool_en_extract_log.csv",
                ...
                append_log_row(
                    paths.extract_log_csv,
                    {
                        **base,
                        ...
                        "error": "",
                        "input_words": int(count_words(profile_text)),
                ...
                append_log_row(
                    paths.extract_log_csv,
                    {
                        **base,
                        "status": "error",
                        "error": str(e),
                        "input_words": int(count_words(read_profile_text(in_path)))
                        if in_path.exists()
                        else 0,
                        "output_words": 0,
                    },
