...
def main() -> None:
    ...
    paths = Paths(pool_en_raw_dir=project_root / 'data/raw/POOL-EN', query_profiles_dir=project_root / 'data/esrc/pool_en/query_profiles', candidate_profiles_dir=project_root / 'data/esrc/pool_en/candidate_profiles', manifest_csv=project_root / 'results/tables/pool_en_profile_manifest.csv')
    ...
    if not paths.pool_en_raw_dir.exists():
        raise FileNotFoundError(f'POOL-EN input folder not found: {paths.pool_en_raw_dir}\nPlace POOL-EN JSONL files there and rerun.')
    ...
    for user_id in all_user_ids:
        ...
        manifest_rows.append({'user_id': user_id, 'role': role, 'has_query': int(has_query), 'has_candidate': int(has_candidate), 'n_query_comments': int(len(q_lines)), 'n_candidate_comments': int(len(c_lines)), 'query_words': int(count_words_for_lines(q_lines)), 'candidate_words': int(count_words_for_lines(c_lines))})
