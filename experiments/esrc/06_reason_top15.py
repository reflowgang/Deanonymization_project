...
LOG_FIELDNAMES = ['T_level', 'query_user_id', 'predicted_candidate_user_id', 'confidence', 'reasoning_short', 'status', 'error', 'model', 'prompt_file']
...
def parse_args(argv: Optional[list[str]]=None) -> argparse.Namespace:
    ...
    p.add_argument('--limit', type=int, default=None, help='Limit number of (T_level, query_user_id) groups processed (for testing).')
    p.add_argument('--query_csv', type=str, default=None, help='Optional CSV with columns T_level, query_user_id to process only those groups.')
    p.add_argument('--levels', nargs='+', default=LEVELS_ALL, help='Truncation levels to process, e.g. --levels T1 T8 (default: all).')
    p.add_argument('--model', type=str, default=DEFAULT_MODEL, help='OpenAI model name (default: gpt-4o-mini). Example: --model gpt-4o')
...
def load_done_ok(out_csv: Path) -> set[tuple[str, str]]:
    ...
    return {(str(r['T_level']), str(r['query_user_id'])) for (_, r) in ok_df.iterrows()}
...
def main() -> None:
    ...
    paths = Paths(faiss_csv=project_root / 'results/tables/pool_en_faiss_top15.csv', query_summaries_root=project_root / 'data/esrc/pool_en/summaries', candidate_summaries_dir=project_root / 'data/esrc/pool_en/candidate_summaries', out_csv=project_root / 'results/tables/pool_en_reason_predictions.csv', prompt_file=project_root / PROMPT_REL_PATH)
    ...
    for ((level, query_user_id), g) in try_tqdm(groups, total=len(groups), desc='reason'):
        ...
        try:
            ...
            called_api = True
            out_obj = call_openai_reason(client=client, model=model, prompt_template=prompt_template, query_summary=query_summary, candidate_block=candidate_block)
            ...
            status = 'error'
            error = str(e)
        append_row(paths.out_csv, {'T_level': str(level), 'query_user_id': query_user_id, 'predicted_candidate_user_id': predicted_candidate_user_id, 'confidence': float(confidence), 'reasoning_short': reasoning_short, 'status': status, 'error': error, 'model': model, 'prompt_file': prompt_file_log})
        if status == 'ok':
            done_ok.add(key)
        if called_api:
