...
RESULT_COLUMNS = [
    ...
    "n_total",
    "n_correct",
    "recall_at_90_original",
    "recall_at_90_calibrated_per_level",
    "recall_at_90_calibrated_global",
    "recall_at_99_original",
    "recall_at_99_calibrated_per_level",
    "recall_at_99_calibrated_global",
    "ece_original",
    "ece_calibrated",
    "auc_pr_original",
    "auc_pr_calibrated",
...
SUMMARY_COLUMNS = [
    "dataset",
    "weighted_recall_90_original",
    "weighted_recall_90_calibrated",
    "weighted_recall_99_original",
    "weighted_recall_99_calibrated",
    "weighted_ece_original",
    "weighted_ece_calibrated",
    "weighted_auc_pr_original",
    "weighted_auc_pr_calibrated",
    "global_temperature",
]

PROTECTED_FILES = [
    "results/tables/hn_recall_at_precision.csv",
    "results/tables/pool_en_recall_at_precision.csv",
    "results/tables/hn_precision_recall_curve.csv",
    "results/tables/pool_en_precision_recall_curve.csv",
    "paper.tex",
]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Post-hoc temperature scaling calibration (exploratory; writes new files only)."
    )
    p.add_argument(
        "--dataset",
        choices=["reddit", "hn", "both"],
        default="both",
        help="Dataset(s) to calibrate (default: both).",
    )
    return p.parse_args()
...
def fit_temperature(probs: np.ndarray, labels: np.ndarray) -> float:
    ...
    def nll(temperature: float) -> float:
        ...
        loss = -(
            labels * np.log(calibrated) + (1.0 - labels) * np.log(1.0 - calibrated)
        )
...
def compute_metrics(y_true: np.ndarray, y_score: np.ndarray) -> dict[str, float]:
    if len(y_true) == 0:
        return {
            "recall_at_90": float("nan"),
            "recall_at_99": float("nan"),
            "ece": float("nan"),
            "auc_pr": float("nan"),
        }
    return {
        "recall_at_90": recall_at_precision(y_true, y_score, 0.90),
        "recall_at_99": recall_at_precision(y_true, y_score, 0.99),
...
def load_reddit_predictions(path: Path) -> pd.DataFrame:
    ...
    df["is_correct"] = (
        df["query_user_id"].astype(str) == df["predicted_candidate_user_id"].astype(str)
    ).astype(int)
...
def plot_dataset_figure(
    ...
    test_calibrated_global: np.ndarray,
    ...
    ax.hist(
        test_calibrated_global,
        bins=20,
        alpha=0.55,
        density=True,
        label="Calibrated (global)",
        color="#C44E52",
    )
...
def process_dataset(config: DatasetConfig) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    global_temperature = fit_temperature(
        np.concatenate(pooled_cal_probs),
        np.concatenate(pooled_cal_labels),
    )
    ...
    for level in LEVELS_ORDER:
        ...
        row = {
            ...
            "n_correct": int(y_test.sum()),
            "recall_at_90_original": orig_metrics["recall_at_90"],
            "recall_at_90_calibrated_per_level": level_metrics["recall_at_90"],
            "recall_at_90_calibrated_global": global_metrics["recall_at_90"],
            "recall_at_99_original": orig_metrics["recall_at_99"],
            "recall_at_99_calibrated_per_level": level_metrics["recall_at_99"],
            "recall_at_99_calibrated_global": global_metrics["recall_at_99"],
            "ece_original": orig_metrics["ece"],
            "ece_calibrated": global_metrics["ece"],
            "auc_pr_original": orig_metrics["auc_pr"],
            "auc_pr_calibrated": global_metrics["auc_pr"],
            "delta_recall_90": global_metrics["recall_at_90"] - orig_metrics["recall_at_90"],
            "delta_recall_99": global_metrics["recall_at_99"] - orig_metrics["recall_at_99"],
            "delta_ece": global_metrics["ece"] - orig_metrics["ece"],
            "delta_auc_pr": global_metrics["auc_pr"] - orig_metrics["auc_pr"],
            "temperature_per_level": temp_level,
            "temperature_global": global_temperature,
        }
        rows.append(row)
    ...
    summary = {
        "dataset": config.name,
        "weighted_recall_90_original": weighted_mean(
            results_df["recall_at_90_original"], weights
        ),
        "weighted_recall_90_calibrated": weighted_mean(
            results_df["recall_at_90_calibrated_global"], weights
        ),
        "weighted_recall_99_original": weighted_mean(
            results_df["recall_at_99_original"], weights
        ),
        "weighted_recall_99_calibrated": weighted_mean(
            results_df["recall_at_99_calibrated_global"], weights
        ),
        "weighted_ece_original": weighted_mean(results_df["ece_original"], weights),
        "weighted_ece_calibrated": weighted_mean(results_df["ece_calibrated"], weights),
        "weighted_auc_pr_original": weighted_mean(results_df["auc_pr_original"], weights),
        "weighted_auc_pr_calibrated": weighted_mean(results_df["auc_pr_calibrated"], weights),
...
def print_assessment(summaries: list[dict[str, float]]) -> None:
    ...
    for summary in summaries:
        ...
        mean_recall_90_inc = 100.0 * (
            summary["weighted_recall_90_calibrated"] - summary["weighted_recall_90_original"]
        )
        mean_recall_99_inc = 100.0 * (
            summary["weighted_recall_99_calibrated"] - summary["weighted_recall_99_original"]
        )
        mean_auc_inc = summary["weighted_auc_pr_calibrated"] - summary["weighted_auc_pr_original"]

        all_recall_improvements_pp.extend(
            [abs(mean_recall_90_inc), abs(mean_recall_99_inc)]
        )
    ...
    if all(v < 1.0 for v in all_recall_improvements_pp):
        print(
            '\nTemperature scaling improves probability calibration but has negligible '
            'impact on deanonymization performance.'
        )
...
def main() -> None:
    ...
    configs = [
        DatasetConfig(
            name="reddit",
            display_name="Reddit",
            predictions_csv=root / "results/tables/pool_en_reason_predictions_clean.csv",
            results_csv=root / "results/tables/pool_en_calibration_results.csv",
            figure_path=root / "results/figures/calibration_before_after_reddit.png",
            success_status="ok",
            level_col="T_level",
        ),
        DatasetConfig(
            ...
            predictions_csv=root / "results/tables/hn_reason_predictions.csv",
            results_csv=root / "results/tables/hn_calibration_results.csv",
            figure_path=root / "results/figures/calibration_before_after_hn.png",
