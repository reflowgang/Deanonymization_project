...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Extract summaries from POOL-EN candidate profiles using a prompt file."
    )
    p.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Process only the first N candidate files (for testing).",
    )
...
def try_tqdm(iterable: Iterable[Path], total: int, desc: str) -> Iterable[Path]:
    try:
        ...
        return tqdm(iterable, total=total, desc=desc)  # type: ignore[return-value]
    except Exception:
        return iterable
...
def log_row_base(
    ...
    out_path: Path,
    prompt_file: str,
    original_input_words: int,
    sent_input_words: int,
    was_truncated: bool,
) -> dict[str, object]:
    return {
        "user_id": user_id,
        ...
        "original_input_words": int(original_input_words),
        "sent_input_words": int(sent_input_words),
        "was_truncated": int(was_truncated),
        "input_words": int(original_input_words),
        "model": MODEL,
...
def main() -> None:
    ...
    paths = Paths(
        ...
        extract_log_csv=project_root / "results/tables/pool_en_candidate_extract_log.csv",
    ...
    if not paths.candidate_profiles_dir.exists():
        raise FileNotFoundError(
            f"Candidate profiles directory not found: {paths.candidate_profiles_dir}"
        )
    ...
    for in_path in try_tqdm(files, total=len(files), desc="candidate"):
        ...
        if should_skip_existing_output(out_path):
            ...
            append_log_row(
                paths.extract_log_csv,
                {
                    **base,
                    "status": "skipped_existing",
                    "error": "",
                    "output_words": int(count_words(summary_text)),
                },
            )
            continue
