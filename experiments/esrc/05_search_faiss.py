...
def user_ids_from_index(df: pd.DataFrame) -> list[str]:
    # Expect row_id to match embedding row order.
    df2 = df.copy()
...
def main() -> None:
    try:
        ...
        raise RuntimeError(
            "faiss is required but not installed. Install faiss-cpu, e.g.\n"
    ...
    paths = Paths(
        ...
        candidate_embeddings_npy=project_root / "data/esrc/pool_en/embeddings/candidate_embeddings.npy",
        candidate_index_csv=project_root / "results/tables/pool_en_candidate_embeddings_index.csv",
        out_csv=project_root / "results/tables/pool_en_faiss_top15.csv",
    )
    ...
    cand_index_df = load_index_csv(
        paths.candidate_index_csv, required_cols=["row_id", "user_id", "summary_path"]
    )
    candidate_user_ids = user_ids_from_index(cand_index_df)

    if cand_emb.shape[0] != len(candidate_user_ids):
        raise ValueError(
            "Candidate embeddings rows do not match candidate index rows: "
            f"{cand_emb.shape[0]} vs {len(candidate_user_ids)}"
        )
    ...
    for level in LEVELS_ALL:
        ...
        q_index_df = load_index_csv(
            q_index_path, required_cols=["row_id", "user_id", "T_level", "summary_path"]
        )
        query_user_ids = user_ids_from_index(q_index_df)

        if q_emb.shape[0] != len(query_user_ids):
            raise ValueError(
                f"{level}: query embeddings rows do not match index rows: "
                f"{q_emb.shape[0]} vs {len(query_user_ids)}"
            )
        if q_emb.shape[1] != cand_emb.shape[1]:
            raise ValueError(
                f"{level}: dim mismatch query={q_emb.shape[1]} candidate={cand_emb.shape[1]}"
            )
    ...
    out_df = pd.DataFrame(
        results_rows, columns=["T_level", "query_user_id", "candidate_user_id", "rank", "score"]
    )
