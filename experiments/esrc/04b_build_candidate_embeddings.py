...
def main() -> None:
    ...
    paths = Paths(candidate_summaries_dir=project_root / 'data/esrc/pool_en/candidate_summaries', embeddings_dir=project_root / 'data/esrc/pool_en/embeddings', out_embeddings_npy=project_root / 'data/esrc/pool_en/embeddings/candidate_embeddings.npy', out_index_csv=project_root / 'results/tables/pool_en_candidate_embeddings_index.csv')
    ...
    it = files
    if tqdm is not None:
        it = tqdm(files, desc='load candidate')
    for p in it:
        text = read_text(p)
        if not text:
            continue
    ...
    index_df = pd.DataFrame({'row_id': list(range(len(user_ids))), 'user_id': user_ids, 'summary_path': summary_paths})
