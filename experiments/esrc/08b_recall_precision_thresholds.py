...
PRECISION_THRESHOLDS = [0.50, 0.60, 0.70, 0.80, 0.90, 0.99]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Recall at multiple precision thresholds by truncation level."
    )
    p.add_argument(
        "--dataset",
        choices=["reddit", "hn", "both"],
        default="both",
        help="Dataset(s) to analyze (default: both).",
    )
    return p.parse_args()
        ...
        long_rows.append(
            {
                ...
                "precision_threshold_pct": int(round(100 * threshold)),
                "recall": recall,
            }
        )
            ...
            long_rows.append(
                {
                    "dataset": config.name,
                    "T_level": level,
                    "n_total": level_wide["n_total"],
                    "n_correct": level_wide["n_correct"],
                    **row,
                }
            )
...
def main() -> None:
    ...
    configs = [
        DatasetConfig(
            ...
            long_csv=root / "results/tables/pool_en_recall_precision_thresholds.csv",
            wide_csv=root / "results/tables/pool_en_recall_precision_thresholds_wide.csv",
            figure_path=root / "results/figures/recall_vs_precision_threshold_reddit.png",
        ),
        DatasetConfig(
            name="hn",
            display_name="Hacker News",
            predictions_csv=root / "results/tables/hn_reason_predictions.csv",
            long_csv=root / "results/tables/hn_recall_precision_thresholds.csv",
            wide_csv=root / "results/tables/hn_recall_precision_thresholds_wide.csv",
            figure_path=root / "results/figures/recall_vs_precision_threshold_hn.png",
        ),
    ]
