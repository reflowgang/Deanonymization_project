...
def main() -> None:
    ...
    paths = Paths(query_profiles_dir=project_root / 'data/esrc/pool_en/query_profiles', truncated_root=project_root / 'data/esrc/pool_en/truncated_queries', stats_csv=project_root / 'results/tables/pool_en_truncation_stats.csv')
    ...
    for in_path in try_tqdm(input_files, total=total_inputs):
        ...
        for (level, n_take) in TRUNCATION_LEVELS:
            ...
            stats_rows.append({'user_id': user_id, 'T_level': level, 'n_comments': int(len(selected)), 'n_words': int(count_words_in_lines(selected))})
