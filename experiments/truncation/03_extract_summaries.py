...
def parse_args(argv: Optional[list[str]]=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Extract summaries from HN user profiles using the Lermen G.2 prompt.')
    p.add_argument('--levels', nargs='+', default=LEVELS_ALL, help='Levels to process, e.g. --levels T1 T2 (default: all).')
    p.add_argument('--limit', type=int, default=None, help='Limit number of users per level (for testing).')
...
def main() -> None:
    ...
    paths = Paths(summaries_root=project_root / 'data/summaries', extracted_root=project_root / 'data/extracted_summaries', extract_log_csv=project_root / 'results/tables/hn_extract_log.csv', prompt_file=project_root / PROMPT_REL_PATH)
    ...
    for level in levels:
        ...
        for profile_path in profiles:
            ...
            if out_path.exists():
                log_rows.append({'level': level, 'user_id': user_id, 'input_path': str(profile_path), 'output_path': str(out_path), 'input_words': None, 'status': 'skipped_exists', 'error': ''})
                continue
            text = profile_path.read_text(encoding='utf-8')
            input_words = count_words(text)
            if input_words == 0:
                log_rows.append({'level': level, 'user_id': user_id, 'input_path': str(profile_path), 'output_path': str(out_path), 'input_words': int(input_words), 'status': 'skipped_empty', 'error': ''})
                continue
            try:
                if client is None:
                ...
                log_rows.append({'level': level, 'user_id': user_id, 'input_path': str(profile_path), 'output_path': str(out_path), 'input_words': int(input_words), 'status': 'success', 'error': ''})
                time.sleep(SLEEP_SECONDS)
            except Exception as e:
                log_rows.append({'level': level, 'user_id': user_id, 'input_path': str(profile_path), 'output_path': str(out_path), 'input_words': int(input_words), 'status': 'error', 'error': str(e)})
