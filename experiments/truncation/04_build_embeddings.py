...
def main() -> None:
    ...
    paths = Paths(extracted_root=project_root / 'data/extracted_summaries', embeddings_root=project_root / 'data/embeddings', embeddings_index_csv=project_root / 'results/tables/hn_embeddings_index.csv')
    ...
    for level in levels:
        ...
        for summary_path in summary_files:
            ...
            if out_path.exists():
                index_rows.append({'user_id': user_id, 'level': level, 'path': str(out_path), 'vector_dim': load_embedding_dim(out_path)})
                continue
            ...
            index_rows.append({'user_id': user_id, 'level': level, 'path': str(out_path), 'vector_dim': int(vec.shape[0])})
