...
def normalize_and_sort(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep text as string; drop rows with missing author or text
...
def row_to_record(row: pd.Series, user_id: str) -> dict:
    ...
    return {
        ...
        "text": row["text"],
        "timestamp": ts_str,
        "unix_time": int(row["unix_time"]) if pd.notna(row["unix_time"]) else None,
        "id": row["id"],
        "parent": row["parent"],
    }
...
def split_user_comments(user_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    n = len(user_df)
    cut = n // 2  # "first 50%" and "second 50%" with floor split
...
def tokenize_words(text: str) -> int:
    # Simple, stable word count for summary stats.
    return len(text.split())


def select_pool_authors(df: pd.DataFrame, min_comments: int) -> tuple[list[str], list[str], list[str]]:
    """Top-N candidate pool by activity; query users sampled randomly (RANDOM_SEED)."""
    authors_sorted = (
        df.groupby("author")
        .size()
    ...
    eligible = [
        ...
        if int((df["author"] == author).sum()) >= min_comments
    ]
    if len(eligible) < N_CANDIDATE_USERS:
        raise ValueError(
            f"Need at least {N_CANDIDATE_USERS} authors with >= {min_comments} comments; "
            f"found {len(eligible):,}."
        )

    # Candidate pool = top 1000 eligible users by comment count.
    candidate_authors = eligible[:N_CANDIDATE_USERS]

    # Query users = random 500 from the candidate pool (seed 42).
    rng = random.Random(RANDOM_SEED)
    query_authors = sorted(
        rng.sample(candidate_authors, N_QUERY_USERS), key=candidate_authors.index
    )
    query_author_set = set(query_authors)
    # Remaining 500 = candidate-only distractors.
    distractor_authors = [a for a in candidate_authors if a not in query_author_set]

    if len(query_authors) != N_QUERY_USERS:
        raise ValueError(
            f"Expected {N_QUERY_USERS} query authors, got {len(query_authors):,}."
        )
    if len(distractor_authors) != N_DISTRACTOR_USERS:
        raise ValueError(
            f"Expected {N_DISTRACTOR_USERS} distractor authors, got {len(distractor_authors):,}."
        )
    return candidate_authors, query_authors, distractor_authors


def validate_pool_author_partition(
    candidate_authors: list[str],
    query_authors: list[str],
    distractor_authors: list[str],
) -> None:
    ...
    if query_set | distractor_set != candidate_set:
        raise ValueError(
            "query_authors and distractor_authors must partition candidate_authors."
        )


def validate_pool_outputs(
    manifest_df: pd.DataFrame,
    query_user_ids: set[str],
    paths: Paths,
) -> None:
    if len(manifest_df) != N_CANDIDATE_USERS:
        raise ValueError(
            f"Manifest must have {N_CANDIDATE_USERS} rows, got {len(manifest_df):,}."
        )

    if int(manifest_df["has_candidate"].sum()) != N_CANDIDATE_USERS:
        raise ValueError(
            "Expected has_candidate == 1 for all 1000 pool users."
        )

    if int(manifest_df["has_query"].sum()) != N_QUERY_USERS:
        raise ValueError(
            f"Expected has_query == 1 for {N_QUERY_USERS} users, "
            f"got {int(manifest_df['has_query'].sum()):,}."
        )

    if int(manifest_df["is_distractor"].sum()) != N_DISTRACTOR_USERS:
        raise ValueError(
            f"Expected is_distractor == 1 for {N_DISTRACTOR_USERS} users, "
            f"got {int(manifest_df['is_distractor'].sum()):,}."
        )
    ...
    distractor_ids = set(
        manifest_df.loc[manifest_df["is_distractor"] == 1, "user_id"].astype(str)
    )
    ...
    for level, _requested in TRUNCATION_LEVELS:
        ...
        if n_files != N_QUERY_USERS:
            raise ValueError(
                f"Truncation level {level} must contain exactly {N_QUERY_USERS} query files; "
                f"found {n_files:,} in {out_dir}."
            )
...
def main() -> None:
    ...
    paths = Paths(
        raw_csv=project_root / "data/raw/hn_comments_active_users_1500.csv",
        splits_query_full_dir=project_root / "data/splits/query_full",
        splits_candidate_dir=project_root / "data/splits/candidate",
        mapping_csv=project_root / "data/filtered/hn_user_mapping.csv",
        pool_manifest_csv=project_root / "data/filtered/hn_pool_manifest.csv",
        truncated_root=project_root / "data/truncated",
        truncation_summary_csv=project_root / "results/tables/hn_truncation_counts.csv",
    )
    ...
    min_comments = 20
    candidate_authors, query_authors, distractor_authors = select_pool_authors(
        df, min_comments=min_comments
    )
    query_author_set = set(query_authors)
    ...
    print(
        f"POOL setup (min_comments={min_comments}): "
        f"candidate pool = top {N_CANDIDATE_USERS} eligible users by activity; "
        f"query users = random {N_QUERY_USERS} from pool (seed {RANDOM_SEED}); "
        f"distractors = remaining {N_DISTRACTOR_USERS} candidate-only users."
    )
    ...
    for i, author in enumerate(candidate_authors, start=1):
        ...
        cand_path = paths.splits_candidate_dir / f"{user_id}_candidate.jsonl"
        c_n = write_jsonl(
            cand_path, (row_to_record(row, user_id) for _, row in c_df.iterrows())
        )
        ...
        if is_query:
            query_path = paths.splits_query_full_dir / f"{user_id}_query_full.jsonl"
            q_n = write_jsonl(
                query_path, (row_to_record(row, user_id) for _, row in q_df.iterrows())
            )
        ...
        manifest_rows.append(
            {
                "user_id": user_id,
                "author": author,
                "role": "query" if is_query else "distractor",
                "has_query": int(is_query),
                "has_candidate": 1,
                "is_distractor": int(is_distractor),
            }
        )
    ...
    for level, requested in TRUNCATION_LEVELS:
        ...
        for user_id, q_df in query_by_user.items():
            ...
            n_written = write_jsonl(
                out_path, (row_to_record(row, user_id) for _, row in t_df.iterrows())
            )
        ...
        trunc_summary_rows.append(
            {
                "level": level,
                "requested_comments": "full" if requested is None else int(requested),
                "n_users_available": int(saved_users),
        ...
        print(
            f"- {level}: requested={('full' if requested is None else requested)}, "
            f"users={saved_users:,}, avg_comments={avg_comments:.2f}, avg_words={avg_words:.2f}"
        )
    ...
    print(
        f"- Candidate pool: top {N_CANDIDATE_USERS:,} eligible users by activity; "
        f"query users: random {N_QUERY_USERS:,} (seed {RANDOM_SEED}); "
        f"distractors: {N_DISTRACTOR_USERS:,} candidate-only"
    )
