...
def parse_args(argv: Optional[list[str]]=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Extract candidate summaries from HN JSONL using the Lermen G.2 prompt.')
    p.add_argument('--limit', type=int, default=None, help='Limit number of users processed (for testing).')
    ...
    with candidate_jsonl_path.open('r', encoding='utf-8') as f:
        for (line_no, line) in enumerate(f, start=1):
            ...
            try:
                ...
                raise ValueError(f'Invalid JSON on line {line_no} in {candidate_jsonl_path}: {e}') from e
            if 'text' not in obj:
                raise ValueError(f'Missing "text" field on line {line_no} in {candidate_jsonl_path}')
...
def main() -> None:
    ...
    paths = Paths(candidate_root=project_root / 'data/splits/candidate', extracted_candidate_root=project_root / 'data/extracted_summaries/candidate', extract_log_csv=project_root / 'results/tables/hn_candidate_extract_log.csv', prompt_file=project_root / PROMPT_REL_PATH)
    ...
    for cand_path in candidate_files:
        ...
        if out_path.exists():
            log_rows.append({'user_id': user_id, 'input_path': str(cand_path), 'output_path': str(out_path), 'input_words': None, 'status': 'skipped_exists', 'error': ''})
            continue
        try:
            ...
            if original_input_words == 0:
                log_rows.append({'user_id': user_id, 'input_path': str(cand_path), 'output_path': str(out_path), 'input_words': 0, 'status': 'skipped_empty', 'error': ''})
                continue
            if client is None:
            ...
            log_rows.append({'user_id': user_id, 'input_path': str(cand_path), 'output_path': str(out_path), 'input_words': int(input_words), 'status': 'success', 'error': ''})
            time.sleep(SLEEP_SECONDS)
        except Exception as e:
            log_rows.append({'user_id': user_id, 'input_path': str(cand_path), 'output_path': str(out_path), 'input_words': None, 'status': 'error', 'error': str(e)})
