...
SUMMARY_KEYS_ORDER: list[str] = ['interests', 'topics', 'writing_style', 'technical_expertise', 'personal_attributes', 'summary']
...
def main() -> None:
    ...
    paths = Paths(extracted_candidate_root=project_root / 'data/extracted_summaries/candidate', embeddings_candidate_root=project_root / 'data/embeddings/candidate', embeddings_index_csv=project_root / 'results/tables/hn_candidate_embeddings_index.csv')
    ...
    for summary_path in summary_files:
        ...
        if out_path.exists():
            index_rows.append({'user_id': user_id, 'path': str(out_path), 'vector_dim': load_embedding_dim(out_path)})
            continue
        ...
        index_rows.append({'user_id': user_id, 'path': str(out_path), 'vector_dim': int(vec.shape[0])})
