...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Reason over FAISS top-15 using the Lermen G.2 record-selection prompt."
    )
    p.add_argument(
        "--levels",
        nargs="+",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Limit number of query users per level (for testing).",
    )
...
def summary_obj_to_text(summary_obj: dict[str, Any]) -> str:
    ...
    # Legacy structured extracts (pre-Lermen JSON schema)
...
def main() -> None:
    ...
    paths = Paths(
        faiss_csv=project_root / "results/tables/hn_faiss_top15.csv",
        query_summaries_root=project_root / "data/extracted_summaries",
        candidate_summaries_dir=project_root / "data/extracted_summaries/candidate",
        out_csv=project_root / "results/tables/hn_reason_predictions.csv",
    ...
    for level in levels:
        ...
        for query_user_id in query_ids:
            ...
            try:
                ...
                query_path = (
                    paths.query_summaries_root / level / f"{query_user_id}_summary.json"
                )
                    ...
                    candidates.append(
                        {
                            ...
                            "summary": summary_obj_to_text(read_json(cpath)),
                        }
                    )
                ...
                predicted_candidate_id, map_err = resolve_predicted_candidate_id(
                    out_obj, cand_ids
                )
                if map_err:
            ...
            is_correct = (
                1
                if (status == "success" and predicted_candidate_id == true_candidate_id)
                else 0
            )
