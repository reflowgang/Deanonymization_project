...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build FAISS index over candidate embeddings and retrieve top-k for each query."
    )
    p.add_argument(
        "--levels",
        nargs="+",
    ...
    p.add_argument(
        ...
        type=int,
        default=15,
        help="Number of candidates to retrieve per query (default: 15).",
    )
...
def l2_normalize_inplace(x: np.ndarray) -> None:
    # Use faiss' L2 normalization for correctness/speed.
...
def recall_at_k(
    query_user_ids: list[str], retrieved_candidate_user_ids: list[list[str]]
) -> float:
...
def main() -> None:
    ...
    paths = Paths(
        candidate_embeddings_dir=project_root / "data/embeddings/candidate",
        query_embeddings_root=project_root / "data/embeddings",
        out_csv=project_root / "results/tables/hn_faiss_top15.csv",
    )
    ...
    if not cand_files:
        ...
        ).to_csv(paths.out_csv, index=False)
    ...
    for level in levels:
        ...
        if query_vectors.shape[1] != candidate_vectors.shape[1]:
            raise ValueError(
                f"{level}: query dim {query_vectors.shape[1]} != candidate dim {candidate_vectors.shape[1]}"
            )
        ...
        for qi, q_user_id in enumerate(query_user_ids):
            ...
            for rank in range(k):
                ...
                results_rows.append(
                    {
                        "level": level,
                        "query_user_id": q_user_id,
                        "rank": rank + 1,
                        "candidate_user_id": cand_user_id,
                        "similarity": sim,
                        "is_true_match": 1 if q_user_id == cand_user_id else 0,
                    }
                )
