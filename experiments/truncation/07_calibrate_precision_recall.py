...
CURVE_COLUMNS = ['level', 'threshold', 'predicted_positive', 'true_positive', 'false_positive', 'false_negative', 'precision', 'recall']
SUMMARY_COLUMNS = ['level', 'n_total', 'n_correct_total', 'top1_accuracy', 'recall_at_90_precision', 'recall_at_99_precision']
...
def compute_pr_curve_for_level(level: str, sub: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    for i in range(n_total):
        ...
        curve_rows.append({'level': level, 'threshold': threshold, 'predicted_positive': predicted_positive, 'true_positive': true_positive, 'false_positive': false_positive, 'false_negative': false_negative, 'precision': precision, 'recall': recall})
    curve_df = pd.DataFrame(curve_rows)
    summary = {'level': level, 'n_total': n_total, 'n_correct_total': n_correct_total, 'top1_accuracy': float(top1_accuracy), 'recall_at_90_precision': recall_at_precision(curve_df, 0.9), 'recall_at_99_precision': recall_at_precision(curve_df, 0.99)}
    return (curve_df, summary)

def main() -> None:
    project_root = Path(__file__).resolve().parents[2]
    paths = Paths(predictions_csv=project_root / 'results/tables/hn_reason_predictions.csv', curve_csv=project_root / 'results/tables/hn_precision_recall_curve.csv', summary_csv=project_root / 'results/tables/hn_recall_at_precision.csv')
    ...
    required = {'level', 'query_user_id', 'predicted_candidate_id', 'true_candidate_id', 'confidence', 'is_correct', 'status', 'error'}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f'Predictions CSV missing columns: {sorted(missing)}')
    df['level'] = df['level'].astype(str)
    df['query_user_id'] = df['query_user_id'].astype(str)
    df['predicted_candidate_id'] = df['predicted_candidate_id'].astype(str)
    df['true_candidate_id'] = df['true_candidate_id'].astype(str)
    df['confidence'] = pd.to_numeric(df['confidence'], errors='coerce').fillna(0.0)
    df = df[df['status'].astype(str) == 'success'].copy()
    if df.empty:
        raise ValueError('No rows with status == "success" in predictions file.')
    ...
    print('\nHN precision-recall calibration (status == success)')
    print('level\tn_total\tn_correct_total\ttop1_accuracy\trecall_at_90_precision\trecall_at_99_precision')
    for (_, row) in summary_out.iterrows():
        print(f"{row['level']}\t{int(row['n_total'])}\t{int(row['n_correct_total'])}\t{row['top1_accuracy']:.4f}\t{row['recall_at_90_precision']:.4f}\t{row['recall_at_99_precision']:.4f}")
