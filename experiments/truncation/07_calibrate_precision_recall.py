...
def main() -> None:
    ...
    paths = Paths(
        predictions_csv=project_root / "results/tables/hn_reason_predictions.csv",
        curve_csv=project_root / "results/tables/hn_precision_recall_curve.csv",
        summary_csv=project_root / "results/tables/hn_recall_at_precision.csv",
    )
    ...
    print(
        "level\tn_total\tn_correct_total\ttop1_accuracy\t"
        "recall_at_90_precision\trecall_at_99_precision"
    )
    for _, row in summary_out.iterrows():
        print(
            f"{row['level']}\t{int(row['n_total'])}\t{int(row['n_correct_total'])}\t"
            f"{row['top1_accuracy']:.4f}\t{row['recall_at_90_precision']:.4f}\t"
            f"{row['recall_at_99_precision']:.4f}"
        )
