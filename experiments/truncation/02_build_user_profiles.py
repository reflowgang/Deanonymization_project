...
def main() -> None:
    ...
    paths = Paths(truncated_root=project_root / 'data/truncated', summaries_root=project_root / 'data/summaries', profile_lengths_csv=project_root / 'results/tables/hn_profile_lengths.csv')
    ...
    for level in TRUNCATION_LEVELS:
        ...
        if not files:
            ...
            summary_rows.append({'level': level, 'n_users': 0, 'avg_characters': 0.0, 'avg_words': 0.0})
            continue
        n_users = 0
        total_chars = 0
        total_words = 0
        ...
        summary_rows.append({'level': level, 'n_users': int(n_users), 'avg_characters': float(avg_chars), 'avg_words': float(avg_words)})
        print(f'- {level}: users={n_users:,}, avg_chars={avg_chars:,.1f}, avg_words={avg_words:,.1f}')
