...
LOG_FIELDNAMES = ['content_type', 'query_user_id', 'predicted_candidate_user_id', 'confidence', 'reasoning_short', 'status', 'error', 'model', 'prompt_file']
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--type', choices=list(type_folders()), default=None, help='Process only one content type.')
...
def load_done_ok(out_csv: Path) -> set[tuple[str, str]]:
    ...
    return {(str(r['content_type']), str(r['query_user_id'])) for (_, r) in ok_df.iterrows()}
...
def main() -> None:
    ...
    for ((content_type, query_user_id), g) in groups:
        ...
        try:
            ...
            status = 'error'
            error = str(e)
        append_row(p['reason_out'], {'content_type': str(content_type), 'query_user_id': query_user_id, 'predicted_candidate_user_id': predicted_candidate_user_id, 'confidence': float(confidence), 'reasoning_short': reasoning_short, 'status': status, 'error': error, 'model': model, 'prompt_file': prompt_file_log})
        if status == 'ok':
            done_ok.add(key)
        if called_api:
