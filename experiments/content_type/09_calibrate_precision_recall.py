...
CURVE_COLUMNS = ['content_type', 'threshold', 'predicted_positive', 'true_positive', 'false_positive', 'false_negative', 'precision', 'recall']
SUMMARY_COLUMNS = ['content_type', 'n_total', 'n_correct_total', 'top1_accuracy', 'recall_at_90_precision', 'recall_at_99_precision']
...
def compute_pr_curve_for_type(content_type: str, sub: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    for i in range(n_total):
        ...
        curve_rows.append({'content_type': content_type, 'threshold': threshold, 'predicted_positive': predicted_positive, 'true_positive': true_positive, 'false_positive': false_positive, 'false_negative': false_negative, 'precision': precision, 'recall': recall})
    curve_df = pd.DataFrame(curve_rows)
    summary = {'content_type': content_type, 'n_total': n_total, 'n_correct_total': n_correct_total, 'top1_accuracy': float(top1_accuracy), 'recall_at_90_precision': recall_at_precision(curve_df, 0.9), 'recall_at_99_precision': recall_at_precision(curve_df, 0.99)}
...
def main() -> None:
    ...
    required = {'content_type', 'query_user_id', 'predicted_candidate_user_id', 'confidence', 'status'}
    ...
    if esrc_summary.exists():
        ...
        qual_summary = qual_summary[~qual_summary['metric'].isin({'top1_accuracy_personal', 'top1_accuracy_opinion', 'top1_accuracy_topical', 'recall_at_90_personal', 'recall_at_90_opinion', 'recall_at_90_topical', 'recall_at_99_personal', 'recall_at_99_opinion', 'recall_at_99_topical'})]
        extra_rows = []
        for (_, row) in summary_out.iterrows():
            ...
            for (folder, key) in TYPE_LABELS.items():
                if TYPE_LABELS[folder] == ct:
                    label = folder
                    break
            extra_rows.extend([{'metric': f'top1_accuracy_{ct}', 'value': round(100.0 * float(row['top1_accuracy']), 2)}, {'metric': f'recall_at_90_{ct}', 'value': round(100.0 * float(row['recall_at_90_precision']), 2) if pd.notna(row['recall_at_90_precision']) else ''}, {'metric': f'recall_at_99_{ct}', 'value': round(100.0 * float(row['recall_at_99_precision']), 2) if pd.notna(row['recall_at_99_precision']) else ''}])
        pd.concat([qual_summary, pd.DataFrame(extra_rows)], ignore_index=True).to_csv(esrc_summary, index=False)
    else:
