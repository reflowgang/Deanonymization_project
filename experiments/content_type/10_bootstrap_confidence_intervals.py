...
N_BOOTSTRAP = 10000
ALPHA = 0.05
TYPE_DISPLAY = {'personal': 'Personal', 'opinion': 'Opinion', 'topical': 'Topical'}
...
def main() -> None:
    ...
    for folder in type_folders():
        ...
        ci_rows.append({'content_type': folder, 'accuracy': round(100.0 * accuracy, 2), 'ci_lower': round(100.0 * ci_lower, 2), 'ci_upper': round(100.0 * ci_upper, 2)})
    bootstrap_out = p['bootstrap_ci']
    with bootstrap_out.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['content_type', 'accuracy', 'ci_lower', 'ci_upper'])
        writer.writeheader()
    ...
    for (type_a, type_b) in combinations(type_folders(), 2):
        ...
        sig_rows.append({'type_a': type_a, 'type_b': type_b, 'p_value': round(p_value, 6), 'significant_0_05': int(p_value < ALPHA)})
    sig_out = p['significance_tests']
    with sig_out.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['type_a', 'type_b', 'p_value', 'significant_0_05'])
        writer.writeheader()
    ...
    for row in ci_rows:
        print(f"  {row['content_type']}: {row['accuracy']:.2f}% [{row['ci_lower']:.2f}, {row['ci_upper']:.2f}]")
    print('\nPairwise two-proportion z-tests:')
    for row in sig_rows:
        sig_label = 'yes' if row['significant_0_05'] else 'no'
        print(f"  {row['type_a']} vs {row['type_b']}: p = {row['p_value']:.6f} (significant at 0.05: {sig_label})")
    print('\nSaved outputs:')
