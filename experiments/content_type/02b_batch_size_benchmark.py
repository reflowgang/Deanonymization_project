...
_spec = importlib.util.spec_from_file_location('classify_comments', Path(__file__).resolve().parent / '02_classify_comments.py')
...
def benchmark_paths(batch_size: int) -> tuple[Path, Path]:
    ...
    return (root / f'content_type_benchmark_bs{batch_size}_classifications.csv', root / f'content_type_benchmark_bs{batch_size}_classify_log.csv')

def run_batch_size(batch_size: int, user_ids: list[str], client, prompt_template: str, raw_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, float]:
    ...
    for user_id in user_ids:
        ...
        for start in range(0, len(comments), batch_size):
            ...
            (labels, log_rows) = c02.classify_batch_with_retry(client=client, model=CLASSIFY_MODEL, prompt_template=prompt_template, user_id=user_id, batch=batch, log_csv=log_csv, prompt_file_log=prompt_file_log)
            for log_row in log_rows:
                c02.append_row(log_csv, c02.LOG_FIELDS, log_row)
            for comment in batch:
                idx = int(comment['comment_index'])
                c02.append_row(classifications_csv, c02.CLASSIFICATION_FIELDS, {'user_id': user_id, 'comment_index': idx, 'timestamp': int(comment['timestamp']), 'source_line': int(comment['source_line']), 'label': labels[idx], 'body': str(comment['body'])})
...
def label_shares(class_df: pd.DataFrame) -> dict[str, float]:
    ...
    return {'personal_share': round(100.0 * counts.get('P', 0) / total, 2), 'opinion_share': round(100.0 * counts.get('O', 0) / total, 2), 'topical_share': round(100.0 * counts.get('T', 0) / total, 2)}

def count_metrics(log_df: pd.DataFrame) -> dict[str, int]:
    return {'api_calls': int((log_df['status'] == 'ok').sum()), 'failures': int((log_df['status'] == 'error').sum()), 'retries': int((log_df['attempt'] > 1).sum() + (log_df['mode'] == 'single_fallback').sum())}

def agreement_rate(left: pd.DataFrame, right: pd.DataFrame) -> float:
    merged = left.merge(right, on=['user_id', 'comment_index'], suffixes=('_a', '_b'), how='inner')
    if merged.empty:
        return 0.0
...
def main() -> None:
    ...
    for batch_size in BATCH_SIZES:
        ...
        (class_df, log_df, runtime_minutes) = run_batch_size(batch_size=batch_size, user_ids=user_ids, client=client, prompt_template=prompt_template, raw_dir=p['raw_query_jsonl'])
        class_by_bs[batch_size] = class_df
        metrics = count_metrics(log_df)
        shares = label_shares(class_df)
        row = {'batch_size': batch_size, 'users': BENCHMARK_USERS, 'comments': int(len(class_df)), 'api_calls': metrics['api_calls'], 'runtime_minutes': round(runtime_minutes, 2), 'failures': metrics['failures'], 'retries': metrics['retries'], **shares}
        results.append(row)
        print(f"runtime={row['runtime_minutes']:.2f} min, api_calls={row['api_calls']}, failures={row['failures']}, retries={row['retries']}, P/O/T={shares['personal_share']}/{shares['opinion_share']}/{shares['topical_share']}%")
    out_csv = p['manifest'].parent / BENCHMARK_CSV
    with out_csv.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['batch_size', 'users', 'comments', 'api_calls', 'runtime_minutes', 'failures', 'retries', 'personal_share', 'opinion_share', 'topical_share'])
        writer.writeheader()
    ...
    print('\n=== Recommendation ===')
    print(f"Fastest: batch_size={fastest['batch_size']} ({fastest['runtime_minutes']:.2f} min, {fastest['api_calls']} API calls)")
    if labels_stable:
        print(f'Labels are stable across batch sizes (agreement >= {min_agreement:.1f}%, share spread <= {max_share_delta:.2f} pp).')
        print(f'Recommend batch_size={recommended} for full classification: best speed with acceptable reliability.')
    else:
        print('Label distributions differ materially across batch sizes; recommend batch_size=10 for maximum labeling consistency.')
    ...
    est_full_calls = int(rec_row['api_calls']) / (BENCHMARK_USERS * 500) * 250000
    est_full_cost = est_full_calls * 0.0005
    print(f'Estimated full-run classification (500 users) at batch_size={recommended}: ~{est_full_hours:.1f} hours, ~{int(est_full_calls):,} API calls, ~${est_full_cost:.2f}.')
