...
LOG_FIELDNAMES = ['content_type', 'user_id', 'input_path', 'output_path', 'status', 'error', 'input_words', 'output_words', 'model', 'prompt_file']

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Extract summaries for P/O/T type profiles.')
    p.add_argument('--type', choices=list(type_folders()), default=None, help='Process only one content type folder.')
...
def main() -> None:
    ...
    for label in CONTENT_TYPES:
        ...
        for in_path in files:
            ...
            base = {'content_type': folder, 'user_id': user_id, 'input_path': str(in_path), 'output_path': str(out_path), 'model': EXTRACT_MODEL, 'prompt_file': prompt_file_log}
            if out_path.exists() and out_path.stat().st_size > 0:
                profile_text = in_path.read_text(encoding='utf-8')
                summary_text = out_path.read_text(encoding='utf-8')
                append_log_row(p['extract_log'], {**base, 'status': 'skipped_existing', 'error': '', 'input_words': count_words(profile_text), 'output_words': count_words(summary_text)})
                continue
            try:
                profile_text = in_path.read_text(encoding='utf-8')
                input_words = count_words(profile_text)
                if input_words == 0:
                    append_log_row(p['extract_log'], {**base, 'status': 'error', 'error': 'empty_input_profile', 'input_words': 0, 'output_words': 0})
                    continue
                summary_text = call_openai_summary(client, EXTRACT_MODEL, prompt_template, profile_text)
                out_path.write_text(summary_text + ('\n' if summary_text else ''), encoding='utf-8')
                append_log_row(p['extract_log'], {**base, 'status': 'ok', 'error': '', 'input_words': input_words, 'output_words': count_words(summary_text)})
                time.sleep(SLEEP_SECONDS)
            except Exception as e:
                append_log_row(p['extract_log'], {**base, 'status': 'error', 'error': str(e), 'input_words': count_words(in_path.read_text(encoding='utf-8')) if in_path.exists() else 0, 'output_words': 0})
                time.sleep(SLEEP_SECONDS)
