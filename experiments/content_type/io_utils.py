...
def iter_sorted_comments(jsonl_path: Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        for i, (t_val, line_no, body) in enumerate(rows)
    ]
...
def pilot_paths() -> dict[str, Path]:
    ...
    return {
        "classifications_csv": root / "content_type_pilot_classifications.csv",
        "classify_log_csv": root / "content_type_pilot_classify_log.csv",
        "qualification_csv": root / "content_type_pilot_qualification.csv",
        "summary_csv": root / "content_type_pilot_summary.csv",
        "type_profiles_root": data_root,
    }
...
def production_paths() -> dict[str, Path]:
    p = paths()
    return {
        "classifications_csv": p["classifications_csv"],
        "classify_log_csv": p["classify_log_csv"],
        "qualification_csv": p["qualification_csv"],
        "summary_csv": p["summary_out"],
        "type_profiles_root": p["type_profiles_root"],
    }
