...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--min-comments",
        type=int,
        default=MIN_COMMENTS_PER_TYPE,
        help="Minimum comments of a type required to write a profile file.",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    for user_id, user_df in df.groupby("user_id", sort=True):
        ...
        qualifies_all = (
            counts["P"] >= min_comments
            and counts["O"] >= min_comments
            and counts["T"] >= min_comments
        )
