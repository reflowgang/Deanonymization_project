...
def main() -> None:
    ...
    for folder in type_folders():
        ...
        q_index_df = load_index_csv(
            q_index_path, required_cols=["row_id", "user_id", "content_type", "summary_path"]
        )
        query_user_ids = user_ids_from_index(q_index_df)

        if q_emb.shape[0] != len(query_user_ids):
            raise ValueError(
                f"{folder}: query embeddings/index mismatch: "
                f"{q_emb.shape[0]} vs {len(query_user_ids)}"
            )
        if q_emb.shape[1] != cand_emb.shape[1]:
            raise ValueError(
                f"{folder}: dim mismatch query={q_emb.shape[1]} candidate={cand_emb.shape[1]}"
            )
