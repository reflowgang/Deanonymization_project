...
from config import TOP_K, paths, type_folders
...
def main() -> None:
    ...
    cand_index_df = load_index_csv(p['candidate_index'], required_cols=['row_id', 'user_id', 'summary_path'])
    candidate_user_ids = user_ids_from_index(cand_index_df)
    if cand_emb.shape[0] != len(candidate_user_ids):
        raise ValueError(f'Candidate embeddings/index mismatch: {cand_emb.shape[0]} vs {len(candidate_user_ids)}')
    ...
    for folder in type_folders():
        ...
        q_emb_path = type_emb_dir / 'query_embeddings.npy'
        q_index_path = type_emb_dir / 'query_embeddings_index.csv'
        if not q_emb_path.exists() or not q_index_path.exists():
            raise FileNotFoundError(f"Missing embeddings for type '{folder}' in {type_emb_dir}")
        q_emb = load_embeddings(q_emb_path)
        q_index_df = load_index_csv(q_index_path, required_cols=['row_id', 'user_id', 'content_type', 'summary_path'])
        query_user_ids = user_ids_from_index(q_index_df)
        if q_emb.shape[0] != len(query_user_ids):
            raise ValueError(f'{folder}: query embeddings/index mismatch: {q_emb.shape[0]} vs {len(query_user_ids)}')
        if q_emb.shape[1] != cand_emb.shape[1]:
            raise ValueError(f'{folder}: dim mismatch query={q_emb.shape[1]} candidate={cand_emb.shape[1]}')
        ...
        for (qi, query_user_id) in enumerate(query_user_ids):
            ...
            for rank0 in range(k):
                ...
                results_rows.append({'content_type': folder, 'query_user_id': query_user_id, 'candidate_user_id': cand_uid, 'rank': int(rank0 + 1), 'score': float(scores[qi, rank0]), 'is_true_match': int(query_user_id == cand_uid)})
            retrieved_ids_for_recall.append(cand_ids)
        r_at_k = recall_at_k(query_user_ids, retrieved_ids_for_recall)
        type_recalls[folder] = r_at_k
        print(f'- {folder}: n_queries={len(query_user_ids):,}, recall_at_{k}={r_at_k:.4f}')
    ...
    print('\nFAISS Recall@15 by content type:')
    for folder in type_folders():
        print(f'  {folder}: {100.0 * type_recalls[folder]:.1f}%')
