...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--max-retries-single",
        type=int,
        default=DEFAULT_MAX_RETRIES_SINGLE,
        help="Max API attempts per comment in simple single-comment recovery (default: 3).",
    )
    return p.parse_args()
...
def parse_classifications(content: str, batch: list[dict[str, Any]]) -> dict[int, str]:
    ...
    # Accept batch-relative indices 1..N when the model ignores global bracket numbers.
    relative_keys = set(range(1, len(batch) + 1))
    if parsed and set(parsed.keys()) == relative_keys:
        parsed = {
            expected_indices[i - 1]: parsed[i]
...
def call_single_comment_simple(
    client: OpenAI,
    model: str,
...
def classify_missing_comments(
    client: OpenAI,
    model: str,
    user_id: str,
    missing_comments: list[dict[str, Any]],
    max_retries_single: int,
                ...
                log_rows.append(
                    {
                        ...
                        "error": "",
                        "model": model,
                        "prompt_file": SINGLE_PROMPT_FILE_LOG,
                        "n_labels_returned": 1,
                    }
                )
                break
            ...
            log_rows.append(
                {
                    ...
                    "batch_end_index": idx,
                    "batch_size": 1,
                    "attempt": max_retries_single,
                    "mode": "single_simple",
                    "status": FORCED_TOPICAL_STATUS,
                    "error": last_error,
                    "model": model,
                    "prompt_file": SINGLE_PROMPT_FILE_LOG,
                    "n_labels_returned": 1,
    ...
    for user_id in user_ids:
        ...
        for start in range(0, len(comments), batch_size):
            ...
            if not batch:
                continue
            total_batches += 1
            labels, log_rows = classify_batch_with_retry(
                client=client,
            ...
            total_api_calls += persist_batch_results(
                classifications_csv,
                log_csv,
                user_id,
                batch,
            ...
            missing_comments = [
                c for c in batch if int(c["comment_index"]) not in already
            ]
            if missing_comments:
                recovery_labels, recovery_logs = classify_missing_comments(
                    client=client,
                    model=CLASSIFY_MODEL,
                    user_id=user_id,
                    missing_comments=missing_comments,
                    max_retries_single=max_retries_single,
                )
                total_api_calls += persist_batch_results(
                    classifications_csv,
                    log_csv,
