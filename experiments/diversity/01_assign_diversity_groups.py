...
def main() -> None:
    ...
    for user_id in sorted(query_users):
        ...
        rows.append({'user_id': user_id, 'unique_subreddits': unique_subreddits, 'diversity_group': assign_group(unique_subreddits)})
    out_path = p['group_manifest']
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['user_id', 'unique_subreddits', 'diversity_group'])
        writer.writeheader()
