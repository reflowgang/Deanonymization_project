...
def main() -> None:
    ...
    for group in GROUPS:
        ...
        q_index_df = load_index_csv(q_index_path, required_cols=['row_id', 'user_id', 'diversity_group', 'summary_path'])
        query_user_ids = user_ids_from_index(q_index_df)
        if q_emb.shape[0] != len(query_user_ids):
            raise ValueError(f'{group}: query embeddings/index mismatch: {q_emb.shape[0]} vs {len(query_user_ids)}')
        if q_emb.shape[1] != cand_emb.shape[1]:
            raise ValueError(f'{group}: dim mismatch query={q_emb.shape[1]} candidate={cand_emb.shape[1]}')
        ...
        for (qi, query_user_id) in enumerate(query_user_ids):
            ...
            for rank0 in range(k):
                ...
                results_rows.append({'diversity_group': group, 'query_user_id': query_user_id, 'candidate_user_id': cand_uid, 'rank': int(rank0 + 1), 'score': float(scores[qi, rank0]), 'is_true_match': int(query_user_id == cand_uid)})
