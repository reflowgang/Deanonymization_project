...
def main() -> None:
    ...
    for group in GROUPS:
        ...
        it = files
        if tqdm is not None:
            it = tqdm(files, desc=f'load {group}')
        for file_path in it:
        ...
        index_df = pd.DataFrame({'row_id': list(range(len(user_ids))), 'user_id': user_ids, 'diversity_group': [group] * len(user_ids), 'summary_path': summary_paths})
