...
from config import GROUPS, REASON_MODEL, paths
TEMPERATURE = 0
SLEEP_SECONDS = 0.2
...
LOG_FIELDNAMES = ['diversity_group', 'query_user_id', 'predicted_candidate_user_id', 'confidence', 'reasoning_short', 'status', 'error', 'model', 'prompt_file']

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Diversity experiment Reason step (gpt-4o-mini).')
    p.add_argument('--limit', type=int, default=None, help='Limit queries processed (testing).')
    p.add_argument('--group', choices=list(GROUPS), default=None, help='Process only one diversity group.')
...
def build_candidate_block(candidates: list[dict[str, object]]) -> str:
    ...
    for (i, c) in enumerate(candidates, start=1):
        parts.append(f"[{i}] candidate_user_id: {c['candidate_user_id']}\n    rank: {c['rank']}\n    score: {float(c['score']):.6f}\n    summary: {c['summary']}\n")
    return '\n'.join(parts).strip()
...
def load_done_ok(out_csv: Path) -> set[tuple[str, str]]:
    ...
    if df.empty or not {'diversity_group', 'query_user_id', 'status'}.issubset(df.columns):
        return set()
    ok_df = df[df['status'].astype(str) == 'ok']
    return {(str(r['diversity_group']), str(r['query_user_id'])) for (_, r) in ok_df.iterrows()}

def call_openai_reason(client: OpenAI, model: str, prompt_template: str, query_summary: str, candidate_block: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    if not p['faiss_out'].exists():
        raise FileNotFoundError(f"FAISS CSV not found: {p['faiss_out']}. Run 05_search_faiss.py first.")
    ...
    required = {'diversity_group', 'query_user_id', 'candidate_user_id', 'rank', 'score'}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f'FAISS CSV missing columns: {sorted(missing)}')
    if args.group:
        df = df[df['diversity_group'].astype(str) == args.group].copy()
    df = df.sort_values(['diversity_group', 'query_user_id', 'rank'], kind='mergesort')
    groups = list(df.groupby(['diversity_group', 'query_user_id'], sort=False))
    if args.limit is not None:
        groups = groups[:max(0, args.limit)]
    print(f'Queries to process: {len(groups):,}')
    print(f'Model: {model}')
    client: Optional[OpenAI] = None
    for ((diversity_group, query_user_id), g) in groups:
        key = (str(diversity_group), str(query_user_id))
        if key in done_ok:
            continue
        status = 'ok'
        error = ''
        ...
        called_api = False
        try:
            query_path = p['group_summaries_root'] / str(diversity_group) / f'{query_user_id}.txt'
            ...
            for (_, row) in g.iterrows():
                ...
                candidates.append({'candidate_user_id': cid, 'rank': int(row['rank']), 'score': float(row['score']), 'summary': read_text(cand_path)})
                candidate_ids.append(cid)
            if client is None:
                client = get_client(project_root)
            called_api = True
            out_obj = call_openai_reason(client, model, prompt_template, query_summary, build_candidate_block(candidates))
            confidence = clamp01(out_obj.get('confidence', 0.0))
            reasoning_short = str(out_obj.get('reasoning_short', '')).strip()
            (predicted_candidate_user_id, map_err) = resolve_predicted_candidate_id(out_obj, candidate_ids)
            if map_err:
                status = 'error'
                error = map_err
            ...
            status = 'error'
            error = str(e)
        append_row(p['reason_out'], {'diversity_group': str(diversity_group), 'query_user_id': query_user_id, 'predicted_candidate_user_id': predicted_candidate_user_id, 'confidence': float(confidence), 'reasoning_short': reasoning_short, 'status': status, 'error': error, 'model': model, 'prompt_file': prompt_file_log})
        if status == 'ok':
            done_ok.add(key)
        if called_api:
