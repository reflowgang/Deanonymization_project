...
def main() -> None:
    ...
    datasets = {'Reddit / POOL-EN': REDDIT_ACCURACY, 'Hacker News': HN_ACCURACY}
    ...
    for (name, accuracy) in datasets.items():
        ...
        main_results[name] = {'changepoint_index': cp_index, 'changepoint_level': cp_level, 'changepoint_comment_count': cp_comments, 'all_breakpoints_pen2': main_bkps_raw}
    with out_csv.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['dataset', 'changepoint_index', 'changepoint_level', 'changepoint_comment_count'])
        writer.writeheader()
        for (dataset_name, result) in main_results.items():
            writer.writerow({'dataset': dataset_name, 'changepoint_index': result['changepoint_index'], 'changepoint_level': result['changepoint_level'], 'changepoint_comment_count': result['changepoint_comment_count']})
    ...
    ax.scatter([reddit_cp], [REDDIT_ACCURACY[reddit_cp]], color='C0', s=120, zorder=5, edgecolors='black', linewidths=0.8)
    ax.scatter([hn_cp], [HN_ACCURACY[hn_cp]], color='C1', s=120, zorder=5, edgecolors='black', linewidths=0.8, marker='s')
    ax.set_xticks(x)
    ...
    for (dataset_name, result) in main_results.items():
        print(f"{dataset_name}: breakpoints (filtered) = {result['all_breakpoints_pen2']}; MST changepoint index = {result['changepoint_index']}")
