...
HN_CALIB_CONFIDENCE = np.array([0.1, 0.2, 0.3, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95], dtype=float)
HN_CALIB_EMPIRICAL = np.array([0.0, 0.0, 0.0, 0.0, 9.6774, 9.1837, 14.4126, 26.6513, 49.005, 100.0], dtype=float)
...
def build_tables() -> dict[str, pd.DataFrame]:
    ...
    gain_df = pd.DataFrame({'transition': TRANSITIONS, 'reddit_gain': reddit_gain, 'hn_gain': hn_gain})
    reddit_relative = 100.0 * REDDIT_ACCURACY / REDDIT_ACCURACY[-1]
    hn_relative = 100.0 * HN_ACCURACY / HN_ACCURACY[-1]
    relative_df = pd.DataFrame({'level': LEVELS, 'comment_count': COMMENT_COUNTS, 'reddit_relative': reddit_relative, 'hn_relative': hn_relative})
    calib_df = pd.DataFrame({'confidence': HN_CALIB_CONFIDENCE, 'empirical_accuracy': HN_CALIB_EMPIRICAL, 'count': HN_CALIB_COUNTS})
    search_reason_df = pd.DataFrame({'level': LEVELS, 'comment_count': COMMENT_COUNTS, 'faiss_recall_at_15': HN_FAISS_RECALL_AT_15, 'gpt4o_top1_accuracy': HN_ACCURACY})
    gap_df = pd.DataFrame({'level': LEVELS, 'comment_count': COMMENT_COUNTS, 'reddit_accuracy': REDDIT_ACCURACY, 'hn_accuracy': HN_ACCURACY, 'reddit_minus_hn': REDDIT_ACCURACY - HN_ACCURACY})
    return {'accuracy_gain_per_step': gain_df, 'relative_deanonymization_power': relative_df, 'hn_confidence_calibration': calib_df, 'search_vs_reason_hn': search_reason_df, 'reddit_hn_accuracy_gap': gap_df}
...
def plot_confidence_calibration_hn() -> plt.Figure:
    ...
    ax.scatter(HN_CALIB_CONFIDENCE, HN_CALIB_EMPIRICAL, s=sizes, alpha=0.75, edgecolors='black', linewidths=0.5, label='Empirical accuracy')
...
def plot_search_vs_reason_hn() -> plt.Figure:
    x = np.arange(len(LEVELS))
    (fig, ax) = plt.subplots(figsize=(10, 6))
    ax.plot(x, HN_FAISS_RECALL_AT_15, marker='o', linewidth=2, label='Search stage: FAISS Recall@15')
    ax.plot(x, HN_ACCURACY, marker='s', linewidth=2, label='Reason stage: GPT-4o Top-1 Accuracy')
    ax.set_xticks(x)
    ax.set_xticklabels(format_level_labels())
...
def main() -> None:
    ...
    tables = build_tables()
    table_files = {'accuracy_gain_per_step': tables_dir / 'accuracy_gain_per_step.csv', 'relative_deanonymization_power': tables_dir / 'relative_deanonymization_power.csv', 'hn_confidence_calibration': tables_dir / 'hn_confidence_calibration.csv', 'search_vs_reason_hn': tables_dir / 'search_vs_reason_hn.csv', 'reddit_hn_accuracy_gap': tables_dir / 'reddit_hn_accuracy_gap.csv'}
    for (key, path) in table_files.items():
        tables[key].to_csv(path, index=False)
        saved_paths.append(path)
    figure_specs = [('accuracy_gain_per_step', plot_accuracy_gain_per_step), ('relative_deanonymization_power', plot_relative_deanonymization_power), ('confidence_calibration_hn', plot_confidence_calibration_hn), ('search_vs_reason_hn', plot_search_vs_reason_hn), ('reddit_hn_accuracy_gap', plot_reddit_hn_accuracy_gap)]
