...
def load_reddit_correctness(project_root: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    df['is_correct'] = (df['predicted_candidate_user_id'].astype(str) == df['query_user_id'].astype(str)).astype(int)
...
def compute_bootstrap_rows(dataset: str, df: pd.DataFrame, rng: np.random.Generator) -> list[dict[str, object]]:
    ...
    for level in LEVELS:
        ...
        rows.append({'dataset': dataset, 'level': level, 'accuracy': round(100.0 * accuracy, 2), 'ci_lower': round(100.0 * ci_lower, 2), 'ci_upper': round(100.0 * ci_upper, 2)})
    return rows
...
def main() -> None:
    ...
    with out_csv.open('w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['dataset', 'level', 'accuracy', 'ci_lower', 'ci_upper'])
        writer.writeheader()
    ...
    for row in rows:
        print(f"{row['dataset']} {row['level']}: {row['accuracy']:.2f}% [{row['ci_lower']:.2f}, {row['ci_upper']:.2f}]")
