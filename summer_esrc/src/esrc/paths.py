"""Paths and BSP read-only roots for summer_esrc."""
...
def bsp_root() -> Path:
    """Parent BSP repo (read-only data/prompts)."""
...
def list_hn_query_user_ids(level: str = "T8") -> list[str]:
    """Prefer manifest role=query; fall back to truncated JSONL filenames."""
...
def load_hn_query_profile_text(user_id: str, level: str = "T8") -> str:
    """Load HN truncated JSONL as newline-separated comments (chunking-safe).

    Do NOT use data/summaries/T* — those are space-joined single lines and
    break Extract comment-list chunking.
    """
