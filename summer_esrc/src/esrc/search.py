...
def search_top_k(query_vectors: np.ndarray, candidate_vectors: np.ndarray, candidate_user_ids: Sequence[str], *, k: int=15) -> list[list[SearchHit]]:
    ...
    for i in range(scores.shape[0]):
        ...
        for (rank, j) in enumerate(idx[:k_eff], start=1):
            hits.append(SearchHit(candidate_user_id=str(candidate_user_ids[int(j)]), rank=rank, score=float(row[int(j)])))
        out.append(hits)
    return out
