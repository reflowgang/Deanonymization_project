#!/usr/bin/env python3
"""P4 C2 — bootstrap CI: baseline stack vs cross-tier swap (T8).

Baseline (P1 / weak-Extract / strong-Reason):
  Extract=qwen3.5-4b, Reason=qwen3.6-35b-a3b-nvfp4

Cross-tier (strong-Extract / weak-Reason):
  Extract=qwen3.6-35b-a3b-nvfp4, Reason=qwen3.5-4b

Paired bootstrap n=10_000, seed=2026 (same style as P1).
Delta = cross_tier − baseline.
"""
...
def bootstrap_paired_delta(
    ...
    """Delta = a − b on paired users (cross_tier − baseline)."""
...
def mcnemar_pvalue(a: np.ndarray, b: np.ndarray) -> tuple[int, int, int, int, float]:
    """Exact binomial McNemar; a=cross_tier, b=baseline."""
    a = np.asarray(a, dtype=bool)
    b = np.asarray(b, dtype=bool)
    n11 = int((a & b).sum())
    n10 = int((a & ~b).sum())  # cross only
    n01 = int((~a & b).sum())  # baseline only
    n00 = int((~a & ~b).sum())
    n_disc = n10 + n01
    if n_disc == 0:
...
def aligned_vectors(
    a: dict[str, bool], b: dict[str, bool]
...
def main() -> None:
    ...
    p.add_argument(
        "--cross-tier-dir",
        default=str(summer_root() / "results" / "runs" / "p4_cross_tier_full_pool"),
    )
    p.add_argument(
        "--out-dir",
        default=str(summer_root() / "results" / "p4_open_vs_frontier"),
    )
    ...
    for pool in ("pool_en", "hn"):
        ...
        for metric, base_d, xt_d in (
            ("top1", base_top1, xt_top1),
            ("hit_at_15", base_hit, xt_hit),
        ):
            ids, xt_y, base_y = aligned_vectors(xt_d, base_d)
            # Full-set rates (may differ slightly in n)
            ...
            base_p, base_lo, base_hi = bootstrap_rate(
                base_full, n_boot=args.n_boot, seed=args.seed
            )
            xt_p, xt_lo, xt_hi = bootstrap_rate(
                xt_full, n_boot=args.n_boot, seed=args.seed
            )
            d_p, d_lo, d_hi = bootstrap_paired_delta(
                xt_y, base_y, n_boot=args.n_boot, seed=args.seed
            )
            n11, n10, n01, n00, p_mc = mcnemar_pvalue(
                xt_y.astype(bool), base_y.astype(bool)
            )
            ...
            rows.append(
                {
                    "pool": pool,
                    "metric": metric,
                    "n_baseline": len(base_ids),
                    "n_cross_tier": len(xt_ids),
                    "n_paired": len(ids),
                    "baseline_rate": round(base_p, 4),
                    "baseline_ci_low": round(base_lo, 4),
                    "baseline_ci_high": round(base_hi, 4),
                    "baseline_pct": pct(base_p),
                    "cross_tier_rate": round(xt_p, 4),
                    "cross_tier_ci_low": round(xt_lo, 4),
                    "cross_tier_ci_high": round(xt_hi, 4),
                    "cross_tier_pct": pct(xt_p),
                    "delta_cross_minus_base": round(d_p, 4),
                    "delta_ci_low": round(d_lo, 4),
                    "delta_ci_high": round(d_hi, 4),
                    "delta_pct_pts": round(100.0 * d_p, 2),
                    "delta_ci_excludes_zero": significant,
                    "mcnemar_n11": n11,
                    "mcnemar_n10_cross_only": n10,
                    "mcnemar_n01_base_only": n01,
                    "mcnemar_n00": n00,
            ...
            print(
                f"{pool} {metric}: base={pct(base_p)}% [{pct(base_lo)}-{pct(base_hi)}] "
                f"cross={pct(xt_p)}% [{pct(xt_lo)}-{pct(xt_hi)}] "
                f"Δ={100 * d_p:+.1f}pp [{100 * d_lo:+.1f},{100 * d_hi:+.1f}] "
                f"excludes0={significant} McNemar p={p_mc:.4g} paired={len(ids)}"
            )
        ...
        compact.append(
            {
                ...
                "baseline": (
                    f"{r['baseline_pct']}% "
                    f"[{pct(r['baseline_ci_low'])}-{pct(r['baseline_ci_high'])}]"
                ),
                "cross_tier": (
                    f"{r['cross_tier_pct']}% "
                    f"[{pct(r['cross_tier_ci_low'])}-{pct(r['cross_tier_ci_high'])}]"
                ),
                "delta_pp": (
                    f"{r['delta_pct_pts']:+.1f} "
                    f"[{100 * r['delta_ci_low']:+.1f}, {100 * r['delta_ci_high']:+.1f}]"
                ),
