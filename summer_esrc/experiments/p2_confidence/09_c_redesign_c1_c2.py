#!/usr/bin/env python3
"""P2 bonus — redesigned estimator (c): C1 binary confirm + C2 pairwise top-3.

Reuses existing Reason picks (same pick as a/b). No re-Extract / re-Search.
Smoke with --limit 10 first; then full regression_50.

Scores:
  C1: P(YES) from first-token YES/NO logprobs
  C2: mean_j P(A beats rival j) over top-3 Search rivals (by rank, ≠ pick)
"""
...
from esrc.confidence import binary_choice_probability  # noqa: E402
...
from esrc.reason_prompt import load_prompt_template  # noqa: E402

DEFAULT_MODEL = "qwen3.6-35b-a3b-nvfp4"
DEFAULT_REASON = (
    summer_root() / "results" / "runs" / "p2_regression_50_T8" / "reason_predictions.jsonl"
)
...
def cand_by_number(pack: dict, number: int) -> dict:
    # candidates are 1-indexed by rank in Reason prompts
...
def one_shot_binary(
    *,
    ...
    out: dict[str, Any] = {
        "raw_text": (result.text or "")[:80],
        "finish_reason": result.finish_reason,
        "n_token_logprobs": len(result.token_logprobs),
        "first_token": result.token_logprobs[0].token if result.token_logprobs else "",
        "first_token_top": [
            {"token": t, "logprob": lp}
            for t, lp in (result.token_logprobs[0].top_logprobs if result.token_logprobs else ())
        ][:10],
    }
    if parsed is None:
        out["ok"] = False
...
def main() -> int:
    ...
    tpl_c1 = load_prompt_template(
        summer_root() / "prompts" / "estimator_c1_confirm.txt"
    )
    tpl_c2 = load_prompt_template(
        summer_root() / "prompts" / "estimator_c2_pairwise.txt"
    )
    ...
    print(
        f"C redesign: {len(done)} done, {len(pending)} pending "
        f"(n_rivals={args.n_rivals}, skip_c2={args.skip_c2})",
        flush=True,
    )
    ...
    for row in pending:
        ...
        out: dict[str, Any] = {
            "query_user_id": uid,
            "correct": str(bool(row.get("correct"))),
            "selected_candidate_number": row.get("selected_candidate_number"),
            "selected_candidate_user_id": row.get("selected_candidate_user_id"),
            "score_a": old.get(uid, {}).get("score_a", ""),
            "score_b": old.get(uid, {}).get("score_b", ""),
            "score_c_old": old.get(uid, {}).get("score_c", ""),
            "score_c1_p_yes": "",
            "score_c2_mean_p_a": "",
            "c2_n_rivals": args.n_rivals,
            "c2_min_p_a": "",
            "c1_raw": "",
            "status": "error",
            "error": "",
        ...
        try:
            ...
            prompt_c1 = (
                tpl_c1.replace("{k}", str(pick_num))
                .replace("{query_summary}", q)
                .replace("{candidate_k_summary}", pick["summary"])
            )
            c1 = one_shot_binary(
                client=client,
                model=args.model,
            ...
            if not args.skip_c2:
                ...
                for rival in rivals_for_c2(pack, pick_num, args.n_rivals):
                    j = int(rival["rank"])
                    prompt_c2 = (
                        tpl_c2.replace("{k}", str(pick_num))
                        .replace("{j}", str(j))
                        .replace("{query_summary}", q)
                        .replace("{candidate_k_summary}", pick["summary"])
                        .replace("{candidate_j_summary}", rival["summary"])
                    )
                    c2 = one_shot_binary(
                        client=client,
                        model=args.model,
                    ...
                    if not c2.get("ok"):
                        raise ValueError(
                            f"C2 vs rival {j}: {c2.get('error') or 'failed'}"
                        )
        ...
        print(
            f"{out['status']} {uid} c1={out['score_c1_p_yes']} "
            f"c2={out['score_c2_mean_p_a']} correct={out['correct']} "
            f"{out['latency_s']}s",
            flush=True,
        )

    # Offline summary vs old (c) / (b)
    ...
    if ok:
        ...
        summary = {
            "n_ok": len(ok),
            "n_correct": int(sum(y)),
            "c1_mean": sum(_col("score_c1_p_yes")) / max(1, len(_col("score_c1_p_yes"))),
            "c1_std": None,
            "c2_mean": None,
            "corr_c1_correct": pearson(_col("score_c1_p_yes"), y[: len(_col("score_c1_p_yes"))]),
            "corr_c2_correct": None,
            "corr_b_correct": None,
            "corr_c_old_correct": None,
            "c1_mean_correct": None,
            "c1_mean_incorrect": None,
        }
        ...
        if len(c1) == len(y):
            ...
            summary["c1_mean_correct"] = (
                sum(x for x, yy in zip(c1, y) if yy == 1.0) / max(1, sum(1 for yy in y if yy == 1.0))
            )
            summary["c1_mean_incorrect"] = (
                sum(x for x, yy in zip(c1, y) if yy == 0.0) / max(1, sum(1 for yy in y if yy == 0.0))
            )
            summary["corr_c1_correct"] = pearson(c1, y)
        c2 = _col("score_c2_mean_p_a")
        if len(c2) == len(y) and c2:
            summary["c2_mean"] = sum(c2) / len(c2)
            summary["c2_mean_correct"] = (
                sum(x for x, yy in zip(c2, y) if yy == 1.0) / max(1, sum(1 for yy in y if yy == 1.0))
            )
            summary["c2_mean_incorrect"] = (
                sum(x for x, yy in zip(c2, y) if yy == 0.0) / max(1, sum(1 for yy in y if yy == 0.0))
            )
    ...
    meta = {
        "task": "p2_c_redesign_c1_c2",
        "reason_jsonl": args.reason_jsonl,
        "inputs_dir": args.inputs_dir,
        "model": args.model,
        "n_rivals": args.n_rivals,
        "seed": args.seed,
