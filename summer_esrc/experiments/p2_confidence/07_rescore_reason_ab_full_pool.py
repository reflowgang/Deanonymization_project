#!/usr/bin/env python3
"""P2 — Reason re-score for (a)/(b) on full pools (reuses P1 Extract + Search).

The overnight P1 runner saved pick-only rows (no verbalized confidence /
token logprobs). This script re-runs Reason only, with logprobs=True, writing
P2-shaped reason_predictions.jsonl + estimator_scores.csv for (a)/(b).

Does NOT re-Extract or re-Search. Does NOT run estimator (c).
Uses max_retries=0 and permanent-failure resume skips from the P1 fixes.
"""
...
from esrc.resume import load_ok_user_ids, load_resume_skip_user_ids  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated query_user_ids to run (smoke / subset). "
        "Applied within each selected --pool.",
    )
...
def run_pool(
    ...
    *,
    ...
    # Also skip if scores already ok
    ...
    print(
        f"[{pool}] reason-ab: {n_ok_cached} ok cached, "
        f"{len(done) - n_ok_cached} permanent-skip, {len(pending)} pending "
        f"(K={concurrency})",
        flush=True,
    )
    if not pending:
    ...
    def _one(uid: str) -> dict[str, Any]:
        ...
        try:
            ...
            score_b = (
                logprob_to_unit_interval(id_lp) if id_lp is not None else float("nan")
            )
            correct = pred_id == uid
            ...
            row.update(
                {
                    ...
                    "reasoning_short": str(obj.get("reasoning_short", ""))[:500],
                    "correct": correct,
                    "true_in_top15": true_in,
                    "candidate_user_ids": cand_ids,
                    "sequence_logprob_full": result.sequence_logprob,
                    "selected_id_logprob": id_lp,
        ...
        print(
            f"[{pool}] reason-ab {row['status']} {uid} "
            f"pick={row.get('selected_candidate_number')} "
            f"conf={row.get('verbalized_confidence')} "
            f"correct={row.get('correct')} {lat:.1f}s",
            flush=True,
        )
        return row
...
def main() -> int:
    ...
    for pool in pools:
        ...
        # Per-pool filter: only IDs that belong to this pool's search set
        ...
        run_pool(
            ...
            resume=args.resume,
            limit=args.limit,
            user_ids=pool_filter,
            reason_tpl=reason_tpl,
            load_cand=load_cand,
        )

    (out_dir / "README.md").write_text(
        "# P2 (a)/(b) Reason re-score\n\n"
        f"Created {datetime.now(timezone.utc).isoformat()}\n"
        f"Source P1 extracts/search: `{p1_root}`\n"
