#!/usr/bin/env python3
"""P2.2 — Build regression_50 Reason input packs from BSP FAISS + summaries.

No local P1.4/P1.5 Search/Reason artifacts exist under results/runs/ (only P1.1
smoke). This script therefore reads BSP pool_en FAISS top-15 + T8 summaries
read-only and caches a fixture pack under results/runs/…/inputs/.
"""
...
from esrc.manifests import git_commit, sha256_file, sha256_text, write_manifest  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Default: results/runs/p2_inputs_<fixture>_<level>/",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else (summer_root() / "results" / "runs" / f"p2_inputs_{args.fixture_id}_{level}")
    )
    ...
    for uid in users:
        ...
        for c in cands:
            ...
            enriched.append(
                {
                    **c,
                    "summary": c_path.read_text(encoding="utf-8").strip(),
                    "summary_sha256": sha256_file(c_path),
                }
            )
        pack = {
            ...
            "query_summary_sha256": sha256_file(q_path),
            "candidates": enriched,
            "true_in_top15": any(c["candidate_user_id"] == uid for c in enriched),
        }
    ...
    man = {
        "run_id": out_dir.name,
        "task": "p2.2_build_fixture_reason_inputs",
        "fixture_id": args.fixture_id,
        "T_level": level,
        "n_users": len(packs),
        "source": "bsp_faiss_pool_en",
        "note": (
            "No P1.4/P1.5 local Search artifacts under summer_esrc/results/runs/; "
            "using BSP FAISS top-15 + summaries (read-only)."
        ),
        "faiss_csv": str(faiss_path),
        "faiss_sha256": sha256_file(faiss_path),
        "prompt_file": str(prompt_path),
        "prompt_sha256": sha256_file(prompt_path),
        "user_ids_sha256": sha256_text(
            (summer_root() / "data/fixtures" / args.fixture_id / "user_ids.txt").read_text(
                encoding="utf-8"
            )
        ),
