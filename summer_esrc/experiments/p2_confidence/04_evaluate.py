#!/usr/bin/env python3
"""P2.8 — ECE, Brier, PR / Recall@P for estimators a/b/c on a scores CSV."""
...
from esrc.metrics_calibration import evaluate_scores  # noqa: E402
...
def main() -> int:
    ...
    # (c) diagnostic: how often softmax argmax ≠ P2.3 pick
    ...
    if not c_rows and any(r.get("score_c_argmax_number") not in (None, "") for r in rows):
        # derive if older CSV lacks the flag column
        for r in rows:
            if r.get("score_c_argmax_number") in (None, "") or r.get("selected_candidate_number") in (
                None,
                "",
            ):
                continue
    ...
    # score_c level stats (watch near-uniform / C3 signal)
    ...
    score_c_median = (
        sorted(score_c_vals)[len(score_c_vals) // 2] if score_c_vals else None
    )

    estimators = []
    for key, label in [
        ("score_a", "a_verbalized"),
        ("score_b", "b_selected_id_exp_logprob"),
        ("score_c", "c_softmax_on_selected"),
    ]:
        vals = []
        ok = True
        for r in rows:
        ...
        row_out = {
            ...
            "top1_accuracy": f"{m.top1_accuracy:.6f}",
            "ece": f"{m.ece:.6f}",
            "brier": f"{m.brier:.6f}",
            "average_precision": f"{m.average_precision:.6f}",
            "recall_at_90_precision": (
                "" if m.recall_at_90_precision is None else f"{m.recall_at_90_precision:.6f}"
            ),
            "recall_at_99_precision": (
                "" if m.recall_at_99_precision is None else f"{m.recall_at_99_precision:.6f}"
            ),
            "c_argmax_disagree_n": "",
            "c_argmax_disagree_rate": "",
            "score_c_mean": "",
            "score_c_median": "",
            "score_c_uniform_baseline": "",
        }
        if label.startswith("c_") and disagree_rate is not None:
            ...
            row_out["score_c_median"] = (
                f"{score_c_median:.6f}" if score_c_median is not None else ""
            )
        ...
        if label.startswith("c_") and disagree_rate is not None:
            extra = (
                f" | c_argmax_disagree={n_disagree}/{n_c} ({disagree_rate:.1%})"
                f" | score_c mean={score_c_mean:.4f} median={score_c_median:.4f}"
                f" (uniform=1/15={1/15:.4f})"
            )
        print(
            f"{label}: ECE={m.ece:.4f} Brier={m.brier:.4f} "
            f"AP={m.average_precision:.4f} R@90P={m.recall_at_90_precision}{extra}"
        )
