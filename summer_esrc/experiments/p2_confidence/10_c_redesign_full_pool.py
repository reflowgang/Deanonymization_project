#!/usr/bin/env python3
"""P2 bonus — C1/C2 redesigned (c) on full pools (reuses P2 a/b Reason picks).

Builds query+candidate packs from P1 Extract + Search (no re-Extract).
Old softmax-15 (c) is NOT re-run (15× cost); full-pool compare is vs (a)/(b).

Default K=1, max_retries=0, --resume skips ok + permanent_error.
"""
...
def build_pack(
    ...
    *,
    ...
    return {
        ...
        "query_summary": q,
        "candidates": cands,
        "selected_candidate_number": int(reason["selected_candidate_number"]),
        "selected_candidate_user_id": reason.get("selected_candidate_user_id"),
        "correct": bool(reason.get("correct")),
    }
...
def one_shot_binary(
    *,
    ...
    out.update(
        {"ok": True, "p_positive": p_pos, "lp_positive": lp_pos, "lp_negative": lp_neg}
    )
    return out
...
def run_pool(
    ...
    *,
    ...
    users = sorted(
        uid
        for uid in reason_by
        if uid in ab_by and uid in search_by and (extract_dir / f"{uid}.txt").exists()
    )
    ...
    print(
        f"[{pool}] C1/C2: {len(done)} skip, {len(pending)} pending "
        f"(K={args.concurrency}, rivals={args.n_rivals})",
        flush=True,
    )
    if not pending:
        return
    ...
    def _one(uid: str) -> dict[str, Any]:
        ...
        out: dict[str, Any] = {
            "pool": pool,
            "query_user_id": uid,
            "correct": str(bool(reason.get("correct"))),
            "true_in_top15": str(bool(reason.get("true_in_top15"))),
            "selected_candidate_number": reason.get("selected_candidate_number"),
            "selected_candidate_user_id": reason.get("selected_candidate_user_id"),
            "score_a": ab.get("score_a", ""),
            "score_b": ab.get("score_b", ""),
            "score_c1_p_yes": "",
            "score_c2_mean_p_a": "",
        ...
        try:
            ...
            detail["c1"] = {
                k: c1.get(k) for k in ("ok", "p_positive", "raw_text", "error")
            }
            if not c1.get("ok"):
        ...
        print(
            f"[{pool}] {out['status']} {uid} c1={out['score_c1_p_yes']} "
...
def main() -> int:
    ...
    meta = {
        "task": "p2_c_redesign_full_pool",
        "p1_run_dir": str(p1_root),
        "p2_scores_dir": str(p2_root),
        "model": args.model,
