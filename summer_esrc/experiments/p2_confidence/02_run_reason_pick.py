#!/usr/bin/env python3
"""P2.3 — Single Reason pick (+ verbalized conf + logprobs) on fixture packs.

Produces reason_predictions.jsonl for estimators (a)/(b). Same picks are reused
for (c). Thinking disabled via chat_template_kwargs.enable_thinking=false.
"""
...
from esrc.confidence import estimator_b_selected_id_logprob  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=str(
            summer_root()
            / "results/runs/p2_inputs_regression_50_T8/inputs"
        ),
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Default: results/runs/p2_reason_<timestamp>/",
    )
    p.add_argument(
        "--resume",
        action="store_true",
        help="Skip query_user_ids already ok in existing reason_predictions.jsonl",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    out_dir = (
        ...
        if args.out_dir
            ...
            / "results"
            / "runs"
            / f"p2_reason_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
        )
    )
    ...
    for pack_path in packs:
        ...
        try:
            ...
            if not result.text and result.reasoning_content:
                raise ValueError(
                    "Empty content with non-empty reasoning_content — thinking not disabled?"
                )
            ...
            row.update(
                {
                    ...
                    "reasoning_content_len": len(result.reasoning_content or ""),
                    "finish_reason": result.finish_reason,
                    "status": "ok",
                    "error": "",
                }
            )
            # Persist token logprobs for (b) recompute / audit (compact)
            row["token_logprobs"] = [
                {"token": t.token, "logprob": t.logprob} for t in result.token_logprobs
            ]
            n_ok += 1
