...
from esrc.confidence import estimator_a_verbalized, estimator_b_selected_id_logprob, estimator_c_softmax_mass_on_selected, logprob_to_unit_interval
from esrc.config import load_dotenv_files
from esrc.generate import TokenLogprob, generate, get_client
from esrc.manifests import git_commit, write_manifest
from esrc.paths import bsp_prompt_record_selection, summer_root
from esrc.reason_prompt import build_candidate_block, build_forced_candidate_prompt, load_prompt_template
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--resume', action='store_true', help='Skip query_user_ids already status=ok in estimator_scores.csv')
    return p.parse_args()
...
def score_forced_candidate(*, client, model: str, template: str, pack: dict, candidate_number: int, max_tokens: int, seed: int) -> float:
    block = build_candidate_block(pack['candidates'])
    prompt = build_forced_candidate_prompt(template, pack['query_summary'], block, candidate_number)
    result = generate([{'role': 'user', 'content': prompt}], model=model, client=client, temperature=0.0, max_tokens=max_tokens, logprobs=True, top_logprobs=1, seed=seed, enable_thinking=False)
...
def main() -> int:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else reason_path.parent
    ...
    fieldnames = ['query_user_id', 'correct', 'selected_candidate_number', 'score_a', 'score_b_logprob', 'score_b', 'score_c', 'score_c_argmax_number', 'c_argmax_disagrees_with_pick', 'candidate_scores_json', 'status', 'error']
    ...
    with out_csv.open(mode, newline='', encoding='utf-8') as f:
        ...
        for row in rows:
            ...
            out = {'query_user_id': uid, 'correct': bool(row.get('correct')), 'selected_candidate_number': row.get('selected_candidate_number'), 'score_a': '', 'score_b_logprob': '', 'score_b': '', 'score_c': '', 'score_c_argmax_number': '', 'c_argmax_disagrees_with_pick': '', 'candidate_scores_json': '', 'status': 'error', 'error': ''}
            try:
                ...
                out.update({'score_a': f'{score_a:.6f}', 'score_b_logprob': f'{score_b_lp:.6f}', 'score_b': f'{score_b:.6f}'})
                if not args.skip_c:
                    pack = json.loads(pack_path.read_text(encoding='utf-8'))
                    n_cands = len(pack['candidates'])
                    scores = []
                    for i in range(1, n_cands + 1):
                        lp = score_forced_candidate(client=client, model=args.model, template=template, pack=pack, candidate_number=i, max_tokens=args.max_tokens, seed=args.seed)
                        scores.append(lp)
                        print(f'  {uid} c[{i}/{n_cands}] lp={lp:.4f}', flush=True)
                    (score_c, probs) = estimator_c_softmax_mass_on_selected(scores, sel_num - 1, temperature=1.0)
                    argmax_num = int(max(range(len(probs)), key=lambda i: probs[i])) + 1
                    disagrees = int(argmax_num != sel_num)
                    out.update({'score_c': f'{score_c:.6f}', 'score_c_argmax_number': str(argmax_num), 'c_argmax_disagrees_with_pick': str(disagrees), 'candidate_scores_json': json.dumps(scores)})
                out['status'] = 'ok'
                print(f"ok {uid} a={out['score_a']} b={out['score_b']} c={out['score_c'] or 'skipped'} c_disagree={out['c_argmax_disagrees_with_pick']}", flush=True)
            except Exception as exc:
                out['error'] = f'{type(exc).__name__}: {exc}'
                print(f"ERR {uid}: {out['error']}", flush=True)
            w.writerow(out)
            f.flush()
    write_manifest(out_dir / 'estimators_manifest.json', {'task': 'p2.4_p2.7_estimators', 'reason_jsonl': str(reason_path), 'model': args.model, 'skip_c': args.skip_c, 'enable_thinking': False, 'git_commit': git_commit(summer_root()), 'created_at_utc': datetime.now(timezone.utc).isoformat()})
