#!/usr/bin/env python3
"""P3 — Isotonic calibration on regression_50 half-split (offline validation).

Split regression_50 50/50 with seed=42 (separate from fixture selection seed=2026).
Fit isotonic on cal half; on test half compare:
  - naive: accept if raw conf >= 0.9
  - calibrated: accept if isotonic(conf) >= 0.9
  - also: threshold on cal set targeting precision≈0.9, apply to test

No server calls. Writes to results/p3_calibration/tables/.
"""
...
CAL_SEED = 42  # temperature-scaling / P3 split — NOT fixture seed 2026
...
def run_estimator(
    ...
    # Threshold learned on cal for raw and calibrated scores (target P=0.9)
    thr_raw = threshold_for_precision(conf_cal, y_cal, 0.90)
    thr_cal = threshold_for_precision(
        calibrate_scores(iso, conf_cal), y_cal, 0.90
    )

    rows = []
    # naive blind 0.9 on raw
    for label, scores, thr in [
        ("naive_raw_0.9", conf_test, 0.9),
        ("isotonic_then_0.9", cal_on_test, 0.9),
        ("raw_thr_from_cal", conf_test, thr_raw if thr_raw is not None else 0.9),
        (
            "isotonic_thr_from_cal",
            cal_on_test,
            thr_cal if thr_cal is not None else 0.9,
        ),
    ]:
        ...
        rows.append(
            {
                "estimator": name,
                "policy": label,
                "threshold": fmt(ev.threshold),
                "n_test": len(test_idx),
                "n_accepted": ev.n_accepted,
                "n_correct_accepted": ev.n_correct_accepted,
                "precision": fmt(ev.precision),
                "recall": fmt(ev.recall),
                "cal_threshold_learned": fmt(thr) if "from_cal" in label else "",
            }
        )

    # Per-user test scores for audit
    detail = []
        ...
        detail.append(
            {
                "estimator": name,
                "query_user_id": uid,
                "correct": str(bool(y_test[i])),
                "raw_conf": fmt(float(conf_test[i])),
                "isotonic_conf": fmt(float(cal_on_test[i])),
                "naive_accept": str(bool(conf_test[i] >= 0.9)),
                "isotonic_accept": str(bool(cal_on_test[i] >= 0.9)),
            }
        )

    # Fit quality on cal / test ECE
    ...
    summary = {
        ...
        "thr_raw_for_p90": thr_raw,
        "thr_isotonic_for_p90": thr_cal,
        "test_ece_raw": m_raw_test.ece,
        "test_ece_isotonic": m_iso_test.ece,
        "test_brier_raw": m_raw_test.brier,
        "test_brier_isotonic": m_iso_test.brier,
        "test_r90_raw": m_raw_test.recall_at_90_precision,
        "test_r90_isotonic": m_iso_test.recall_at_90_precision,
    }
...
def main() -> int:
    ...
    for name, conf in estimators:
        ...
        print(
            f"{name}: test ECE raw={summ['test_ece_raw']:.4f} → "
            f"iso={summ['test_ece_isotonic']:.4f}; "
            f"naive P@0.9 accept={pol[0]['n_accepted']} "
            f"prec={pol[0]['precision']} recall={pol[0]['recall']}; "
            f"iso@0.9 accept={pol[1]['n_accepted']} "
            f"prec={pol[1]['precision']} recall={pol[1]['recall']}"
        )
        ...
        flat.append(
            {
                ...
                "n_cal": s["n_cal"],
                "n_test": s["n_test"],
                "cal_n_correct": s["cal_n_correct"],
                "test_n_correct": s["test_n_correct"],
                "thr_raw_for_p90": fmt(s["thr_raw_for_p90"]),
                "thr_isotonic_for_p90": fmt(s["thr_isotonic_for_p90"]),
                "test_ece_raw": fmt(s["test_ece_raw"]),
                "test_ece_isotonic": fmt(s["test_ece_isotonic"]),
                "test_brier_raw": fmt(s["test_brier_raw"]),
                "test_brier_isotonic": fmt(s["test_brier_isotonic"]),
                "test_r90_raw": fmt(s["test_r90_raw"]),
                "test_r90_isotonic": fmt(s["test_r90_isotonic"]),
            }
        )
    ...
    lines = [
        ...
        "",
        f"Split seed={args.seed} (P3 cal/test; **not** fixture seed 2026). "
        f"n_cal={len(cal_idx)}, n_test={len(test_idx)}.",
        "",
    ...
    for r in all_policy:
        if r["policy"] in ("naive_raw_0.9", "isotonic_then_0.9"):
            lines.append(
                f"| {r['estimator']} | {r['policy']} | {r['n_accepted']} | "
    ...
    meta = {
        "task": "p3_isotonic_fixture_validation",
        "scores_csv": str(scores_path),
        "seed": args.seed,
        "n_cal": int(len(cal_idx)),
        "n_test": int(len(test_idx)),
        "cal_user_ids": [user_ids[i] for i in cal_idx],
        "test_user_ids": [user_ids[i] for i in test_idx],
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "note": (
            "Half-split validates code path. Do not cite fixture isotonic "
            "precision/recall as paper numbers; re-run on full pool."
        ),
    }
