#!/usr/bin/env python3
"""
Measure REAL vLLM Extract prompt token counts for POOL-EN profiles using
qwen3.5-4b and the Lermen summarization prompt.

Current selection: 10 profiles spread across ~10k–15k words (T5/T6/T7) to
bracket the true crossover below 16384 tokens.

Also logs template-only overhead once (empty comment body).

Outputs:
  summer_esrc/results/token_counts_bracket_10k_15k.csv
  summer_esrc/results/token_counts_bracket_10k_15k_meta.json

Run:
  cd summer_esrc
  export VLLM_BASE_URL=https://gpu6.sedan.pro/v1
  export VLLM_API_KEY=local-vllm
  ../.venv/bin/python experiments/p1_context/01_token_counts_top10_t7_t8.py
"""
...
WORD_MIN = 10_000
WORD_MAX = 15_000
BUCKET_CENTERS = [10_000, 11_000, 12_000, 13_000, 14_000, 15_000]
PER_BUCKET = 2
N_TOTAL = 10
LEVELS = {"T5", "T6", "T7"}
...
def pick_spread_buckets(
    ...
    *,
    ...
    """Pick profiles in [word_min, word_max], ~per_bucket closest to each bucket center."""
    candidates = [
        r for r in rows if word_min <= int(r["n_words"]) <= word_max
    ]
    if not candidates:
    ...
    for center in bucket_centers:
        ...
        pool = [
            r
            for r in candidates
            if (r["T_level"], r["user_id"]) not in picked_ids
        ]
    ...
    picked.sort(key=lambda r: int(r["n_words"]))  # type: ignore[arg-type]
...
def measure_prompt_tokens(client: OpenAI, user_content: str) -> tuple[str, int | None, int | None, str]:
    try:
        ...
        total_tokens = (
            int(usage.total_tokens) if usage and usage.total_tokens is not None else None
        )
...
def main() -> int:
    ...
    # --- template-only overhead (once) ---
    template_content = build_user_content(prompt_template, "")
    tpl_status, tpl_tokens, tpl_total, tpl_err = measure_prompt_tokens(
        client, template_content
    )
    print("Template-only diagnostic (empty profile_text):")
    print(
        f"  status={tpl_status} prompt_tokens={tpl_tokens} total_tokens={tpl_total}"
        + (f" error={tpl_err!r}" if tpl_err else "")
    )
    print()

    candidates = read_profiles(stats_csv, LEVELS)
    selected = pick_spread_buckets(
        candidates,
        word_min=WORD_MIN,
        word_max=WORD_MAX,
    ...
    print(
        f"Bracket sample: {N_TOTAL} profiles, words in [{WORD_MIN:,}, {WORD_MAX:,}], "
        f"levels {sorted(LEVELS)}, model={EXTRACT_MODEL}, limit={CONTEXT_LIMIT}"
    )
    print("-" * 100)
    ...
    for i, row in enumerate(selected, start=1):
        lvl = str(row["T_level"])
        uid = str(row["user_id"])
        n_words = int(row["n_words"])  # type: ignore[arg-type]
        ...
        status, prompt_tokens, total_tokens, err_str = measure_prompt_tokens(
            client, user_content
        )
        ...
        rows_out.append(
            {
                ...
                "user_id": uid,
                "n_words": n_words,
                "bucket_target_words": bucket_target,
                "status": status,
                "prompt_tokens": prompt_tokens,
                "template_overhead_tokens": tpl_tokens,
                "body_tokens_est": body_tokens_est,
                "tokens_per_word_est": tokens_per_word,
        ...
        print(
            f"{i:>2}. {lvl} {uid} words={n_words:,} bucket~{bucket_target} "
            f"status={status} prompt_tokens={prompt_tokens} "
            f"body_est={body_tokens_est} tok/word={tokens_per_word}"
        )
    ...
    tpw_vals = [float(r["tokens_per_word_est"]) for r in ok_rows]  # type: ignore[arg-type]
    meta = {
        ...
        "template_only": {
            "status": tpl_status,
            "prompt_tokens": tpl_tokens,
            "total_tokens": tpl_total,
            "error": tpl_err,
        },
        "n_profiles": len(rows_out),
        "n_ok": sum(1 for r in rows_out if r["status"] == "ok"),
        "n_exceeds_limit": sum(1 for r in rows_out if r["status"] == "exceeds_limit"),
        "tokens_per_word_est": {
            "mean": round(sum(tpw_vals) / len(tpw_vals), 4) if tpw_vals else None,
            "min": min(tpw_vals) if tpw_vals else None,
            "max": max(tpw_vals) if tpw_vals else None,
        },
    }
    ...
    if tpw_vals:
        print(
            f"tokens_per_word_est (ok only): mean={meta['tokens_per_word_est']['mean']} "
            f"min={meta['tokens_per_word_est']['min']} max={meta['tokens_per_word_est']['max']}"
        )
    return 0
