#!/usr/bin/env python3
"""Production full-pool E→S→R runner: pool_en and/or HN.

Supports Extract/Reason concurrency, --resume (skip status==ok, retry failures),
and per-user incremental writes (summary file + jsonl append under a lock).
"""
...
from esrc.generate import (  # noqa: E402
...
from esrc.paths import (  # noqa: E402
    ...
    bsp_hn_candidate_summaries,
    bsp_hn_pool_manifest,
    bsp_hn_truncated_dir,
    bsp_hn_user_mapping,
    bsp_pool_en,
    bsp_pool_en_candidate_summaries,
    bsp_prompt_record_selection,
    bsp_prompt_summarization,
    bsp_truncated_query,
    list_hn_query_user_ids,
    list_pool_en_query_user_ids,
    load_hn_candidate_summary_text,
    load_hn_query_profile_text,
...
from esrc.resume import (  # noqa: E402
    load_last_ok_by_user,
    load_ok_user_ids,
    load_resume_skip_user_ids,
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--pool",
        choices=("pool_en", "hn", "both"),
        default="both",
        help="Which pool(s) to run (default: both)",
    )
    ...
    p.add_argument(
        "--reason-model",
        default=os.getenv("VLLM_REASON_MODEL", DEFAULT_REASON_MODEL),
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated query_user_ids (smoke / subset), applied per pool",
    )
    p.add_argument(
        "--phase",
        choices=("all", "extract", "search", "reason"),
        default="all",
    )
    ...
    p.add_argument(
        "--out-dir",
        default=None,
        help="Parent run dir; creates pool_en/ and hn/ subdirs",
    )
...
def load_pool_en_candidate_index() -> tuple[list[str], np.ndarray]:
    ...
    if matrix.shape[0] != len(user_ids):
        raise SystemExit(
            f"pool_en embedding rows {matrix.shape[0]} != index {len(user_ids)}"
        )
    return user_ids, matrix


def load_or_build_hn_candidate_index(
    cache_dir: Path,
) -> tuple[list[str], np.ndarray]:
    """Build all-mpnet matrix from HN candidate summary texts (cached).

    BSP HN embeddings are OpenAI text-embedding-3-small (1536-d); summer Search
    uses all-mpnet like pool_en, so we re-embed gallery summaries once.
    """
...
def phase_extract(
    ...
    *,
    ...
    print(
        f"[{pool}] extract: {n_ok} ok cached, {len(done) - n_ok} permanent-skip, "
        f"{len(pending)} pending (K={concurrency})"
    )
    if not pending:
    ...
    def _one(uid: str) -> dict[str, Any]:
        ...
        # Prefer query_user_id for resume compatibility with reason jsonl shape,
        # keep user_id for backwards compat with prior extract_meta rows.
        row: dict[str, Any] = {
            "user_id": uid,
            "query_user_id": uid,
            "pool": pool,
        ...
        try:
            ...
            # Permanent: do not resend unchanged on --resume.
...
def phase_search(
    ...
    *,
    ...
    # Atomic replace so a crash mid-write does not leave a half CSV that
...
def phase_reason(
    ...
    *,
    ...
    concurrency: int,
    timeout: float,
    load_cand_summary: Callable[[str], str],
) -> Path:
    ...
    print(
        f"[{pool}] reason: {n_ok} ok cached, {len(done) - n_ok} permanent-skip, "
        f"{len(pending)} pending (K={concurrency})"
    )
    if not pending:
        return out_jsonl
...
def evaluate(reason_jsonl: Path, extract_meta: dict[str, dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "n_chunked_extract": chunked,
        "n_single_extract": len(extract_meta) - chunked,
    }
...
def run_pool(
    ...
    *,
    ...
    if pool == "pool_en":
        ...
        load_profile = lambda uid: bsp_truncated_query(args.level, uid).read_text(  # noqa: E731
            encoding="utf-8"
        )
        ...
        load_profile = lambda uid: load_hn_query_profile_text(uid, args.level)  # noqa: E731
    ...
    if args.phase in ("all", "search"):
        if pool == "pool_en":
            ...
            cand_ids, cand_matrix = load_or_build_hn_candidate_index(
                out_dir / "cache"
            )
        assert cand_ids is not None and cand_matrix is not None
        # Search users that have summaries (ok extracts), preserving order.
        search_users = [
            u for u in users if (out_dir / "extract" / f"{u}.txt").exists()
        ]
        phase_search(
            search_users,
            pool=pool,
    ...
    if args.phase in ("all", "reason"):
        ...
        phase_reason(
            ...
            concurrency=args.reason_concurrency,
            timeout=args.timeout,
    ...
    if reason_jsonl.exists():
        ...
        print(
            f"[{pool}] metrics top1={metrics['top1_correct']}/{metrics['n_reason_ok']} "
    ...
    write_manifest(
        out_dir / "manifest.json",
        {
            ...
            "reason_concurrency": args.reason_concurrency,
            "resume": args.resume,
            "metrics": metrics,
...
def preflight(pools: list[str], level: str) -> None:
    missing: list[str] = []
    for label, path in [
        ("summarization prompt", bsp_prompt_summarization()),
        ("record_selection prompt", bsp_prompt_record_selection()),
        ("extract_merge prompt", summer_prompt_extract_merge()),
    ]:
        if not path.exists():
    ...
    if "pool_en" in pools:
        checks = [
            bsp_pool_en() / "truncated_queries" / level,
            bsp_pool_en_candidate_summaries(),
            bsp_candidate_embeddings_npy(),
            bsp_candidate_embeddings_index_csv(),
        ]
...
def main() -> int:
    ...
    # Also allow BSP-root env if summer .env missing keys.
    ...
    parent_out = (
        ...
        / "results"
        / "runs"
        / f"p1_full_pool_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    )
    ...
    write_manifest(
        parent_out / "manifest.json",
        {
            "task": "p1_full_pool_parent",
            "pools": pools,
            "T_level": args.level,
            "extract_model": args.extract_model,
            "reason_model": args.reason_model,
