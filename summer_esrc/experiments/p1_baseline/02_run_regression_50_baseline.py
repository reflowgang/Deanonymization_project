...
from esrc.paths import bsp_candidate_embeddings_index_csv, bsp_candidate_embeddings_npy, bsp_pool_en, bsp_prompt_record_selection, bsp_prompt_summarization, fixture_dir, fixture_user_ids, summer_prompt_extract_merge, summer_root
...
def phase_extract(users: list[str], *, level: str, client, extract_model: str, extract_tpl: str, merge_tpl: str, out_dir: Path, resume: bool, max_tokens: int) -> dict[str, dict]:
    ...
    for uid in users:
        ...
        try:
            ...
            row.update({'status': 'ok', 'chunked': result.chunked, 'n_chunks': result.n_chunks, 'method': result.method, 'chunk_sizes': list(result.chunk_sizes), 'probe_tokens_full': result.probe_tokens_full, 'probe_tokens_per_chunk': list(result.probe_tokens_per_chunk), 'summary_chars': len(result.summary), 'n_words': len(text.split())})
...
def phase_search(users: list[str], *, extract_dir: Path, out_dir: Path) -> Path:
...
def phase_reason(users: list[str], *, extract_dir: Path, search_path: Path, client, reason_model: str, reason_tpl: str, out_dir: Path, seed: int, resume: bool, max_tokens: int, fixture_id: str, level: str) -> Path:
    ...
    for uid in users:
        ...
        row: dict = {'fixture_id': fixture_id, 'T_level': level, 'query_user_id': uid, 'status': 'error', 'error': '', 'error_type': '', 'model': reason_model, 'max_tokens': max_tokens}
        try:
...
def evaluate(*, reason_jsonl: Path, extract_meta: dict[str, dict], fixture_id: str) -> dict:
    ...
    metrics = {'n_reason_ok': n, 'top1_correct': top1, 'top1_accuracy': top1 / n if n else 0.0, 'hit_at_15_count': hit15, 'hit_at_15': hit15 / n if n else 0.0, 'n_chunked_extract': len(chunked_users), 'n_single_extract': len(single_users), 'chunked_user_ids': chunked_users, 'single_top1': sum((1 for p in single_ok if p.get('correct'))), 'single_hit15': sum((1 for p in single_ok if p.get('true_in_top15'))), 'chunked_top1': sum((1 for p in chunked_ok if p.get('correct'))), 'chunked_hit15': sum((1 for p in chunked_ok if p.get('true_in_top15'))), 'fixture_reference_top1': ref['top1_correct_count'], 'fixture_reference_hit15': ref['hit_at_15_count'], 'note': 'Fixture baselines used BSP gpt-4o summaries+FAISS; this run uses local Extract+Search. Chunking should not break short profiles; compare pipeline completion and chunk flags.'}
    return metrics
...
def main() -> int:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else summer_root() / 'results' / 'runs' / f"p1_baseline_{args.fixture_id}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    if args.phase in ('all', 'extract'):
        extract_meta = phase_extract(users, level=args.level, client=client, extract_model=args.extract_model, extract_tpl=extract_tpl, merge_tpl=merge_tpl, out_dir=out_dir, resume=args.resume, max_tokens=args.extract_max_tokens)
    ...
    if args.phase in ('all', 'reason'):
        ...
        phase_reason(users, extract_dir=out_dir / 'extract', search_path=search_path, client=client, reason_model=args.reason_model, reason_tpl=reason_tpl, out_dir=out_dir, seed=args.seed, resume=args.resume, max_tokens=args.reason_max_tokens, fixture_id=args.fixture_id, level=args.level)
    metrics = {}
    if reason_jsonl.exists():
        metrics = evaluate(reason_jsonl=reason_jsonl, extract_meta=extract_meta, fixture_id=args.fixture_id)
        (out_dir / 'metrics.json').write_text(json.dumps(metrics, indent=2) + '\n', encoding='utf-8')
        print(f"\nMetrics: top1={metrics['top1_correct']}/{metrics['n_reason_ok']} hit@15={metrics['hit_at_15_count']}/{metrics['n_reason_ok']} chunked={metrics['n_chunked_extract']}")
        print(f"Fixture ref (BSP gpt-4o): top1={metrics['fixture_reference_top1']}/50 hit@15={metrics['fixture_reference_hit15']}/50")
    write_manifest(out_dir / 'manifest.json', {'task': 'p1_baseline_regression_50', 'fixture_id': args.fixture_id, 'T_level': args.level, 'extract_model': args.extract_model, 'reason_model': args.reason_model, 'phase': args.phase, 'n_users': len(users), 'extract_prompt': str(bsp_prompt_summarization()), 'merge_prompt': str(summer_prompt_extract_merge()), 'reason_prompt': str(bsp_prompt_record_selection()), 'metrics': metrics, 'git_commit': git_commit(summer_root()), 'created_at_utc': datetime.now(timezone.utc).isoformat()})
