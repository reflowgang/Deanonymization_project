#!/usr/bin/env python3
"""P1 baseline — smoke-test conditional Extract chunking on known long/borderline profiles."""
...
from esrc.extract import CHUNK_GATE_TOKENS, extract_profile, load_prompt_template  # noqa: E402
...
# Curated from token_counts_bracket_10k_15k.csv + token_counts_top10_t7_t8.csv
DEFAULT_CASES: list[tuple[str, str, str]] = [
    ("T7", "user_0cc626ee", "under_gate_single_chunk"),
    ("T6", "user_378003e3", "borderline_over_gate"),
    ("T7", "user_c709e252", "last_fit_before_overflow"),
    ("T7", "user_e9d1886c", "first_overflow"),
    ("T7", "user_daf37424", "top10_longest"),
    ("T8", "user_daf37424", "top10_longest_t8"),
]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out-dir",
        default=None,
        help="Default: results/runs/p1_chunking_test_<timestamp>/",
    )
...
def main() -> int:
    ...
    out_dir = (
        ...
        / "results"
        / "runs"
        / f"p1_chunking_test_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    )
    ...
    for level, uid, label in DEFAULT_CASES:
        ...
        try:
            ...
            row = {
                ...
                "summary_preview": result.summary[:240],
            }
            (out_dir / f"{uid}_{level}_summary.txt").write_text(result.summary + "\n", encoding="utf-8")
            print(
                f"ok chunked={result.chunked} n_chunks={result.n_chunks} "
                f"method={result.method} probe_full={result.probe_tokens_full} "
                f"sizes={result.chunk_sizes}"
            )
    ...
    write_manifest(
        ...
        {
            "task": "p1_chunking_test",
            "model": args.model,
            "chunk_gate_tokens": CHUNK_GATE_TOKENS,
            "n_cases": len(rows),
