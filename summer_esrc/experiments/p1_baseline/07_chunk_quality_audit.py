#!/usr/bin/env python3
"""P1 — Expanded chunk-quality audit by n_chunks buckets (pool_en + HN).

Buckets: 1 (single-pass), 2–3, 4–6, 7–10, 11+.
Reports top-1 / Hit@15 trend and merge-quality proxies vs archived gpt-4o
summaries (length ratio, trait-count inflation, mid-word truncation).
"""
...
def load_gpt4o_summary(pool: str, uid: str) -> Optional[str]:
    if pool == "hn":
        ...
        return str(obj)
    # pool_en archived query summaries
...
def dup_rate(parts: list[str]) -> float:
    if not parts:
        return 0.0
    norm = [re.sub(r"\s+", " ", p.lower()) for p in parts]
    return 1.0 - (len(set(norm)) / len(norm))


def looks_truncated(summary: str) -> bool:
    """Heuristic for merge cut off mid-generation (not merely unpunctuated lists)."""
    s = summary.rstrip()
    if not s:
        return False
    if s[-1] in ".!?;:\"')]" :
    ...
    if s.endswith("-") or s.endswith("—"):
        return True
    # Long merge dumps that die mid-phrase after a comma
    if len(s) >= 1500 and "," in s:
        last = s.split(",")[-1].strip()
        if last and last[-1].isalnum() and len(last.split()) <= 2:
            return True
    # Very long outputs near typical max_tokens≈1024 English length
...
def main() -> None:
    ...
    for pool in ("pool_en", "hn"):
        ...
        reason = last_ok_jsonl(
            run_dir / pool / "reason_predictions.jsonl",
            id_keys=("query_user_id", "user_id"),
        )
        ...
        for uid, m in meta.items():
            ...
            row = {
                ...
                "user_id": uid,
                "n_chunks": n_chunks,
                "bucket": bucket_label(n_chunks),
                "n_words": m.get("n_words"),
                "local_chars": len(local),
                "gpt4o_chars": len(gpt) if gpt else None,
                "char_ratio_local_over_gpt4o": (len(local) / len(gpt)) if gpt else None,
                "local_traits": len(parts),
                "gpt4o_traits": len(gpt_parts) if gpt_parts else None,
                "dup_rate": round(dup_rate(parts), 4),
                "truncated": looks_truncated(local),
                "top1_correct": bool(r.get("correct")) if r else None,
                "hit_at_15": bool(r.get("true_in_top15")) if r else None,
            }
            per_user.append(row)
        ...
        for label, _ in BUCKETS:
            ...
            bucket_rows.append(
                {
                    ...
                    "bucket": label,
                    "n_users": len(rows),
                    "n_with_reason": len(with_reason),
                    "top1_rate": round(sum(top1) / len(top1), 4) if top1 else None,
                    "top1_pct": round(100 * sum(top1) / len(top1), 1) if top1 else None,
                    "hit_at_15_rate": round(sum(hit) / len(hit), 4) if hit else None,
                    "hit_at_15_pct": round(100 * sum(hit) / len(hit), 1) if hit else None,
                    "mean_local_chars": round(mean(local_chars) or 0, 1),
                    "mean_gpt4o_chars": round(mean([float(x) for x in gpt_chars]) or 0, 1) if gpt_chars else None,
                    "mean_char_ratio": round(mean(ratios) or 0, 2) if ratios else None,
                    "pct_truncated": round(100 * sum(1 for t in trunc if t) / len(trunc), 1) if trunc else None,
                    "mean_dup_rate": round(mean(dups) or 0, 4) if dups else None,
                    "mean_local_traits": round(mean([float(r["local_traits"]) for r in rows]) or 0, 1),
                }
            )

            # sample side-by-sides for write-up (highest n_chunks in bucket first)
            ...
            for r in ranked[: args.samples_per_bucket]:
                ...
                if pool == "hn" and label in ("7-10", "11+", "4-6"):
                    ...
                    cmp.write_text(
                        f"# {uid} ({pool}) n_chunks={r['n_chunks']} bucket={label}\n\n"
                        f"## gpt-4o archived ({len(gpt)} chars)\n\n{gpt}\n\n"
                        f"## local merge ({len(local)} chars, trunc={r['truncated']}, "
                        f"dup={r['dup_rate']}, traits≈{r['local_traits']})\n\n{local}\n",
                        encoding="utf-8",
                    )
    ...
    for row in bucket_rows:
        print(
            f"{row['pool']:8} {row['bucket']:5} n={row['n_users']:3} "
            f"top1={row['top1_pct']}% hit@15={row['hit_at_15_pct']}% "
            f"char_ratio={row['mean_char_ratio']} trunc={row['pct_truncated']}%"
        )
