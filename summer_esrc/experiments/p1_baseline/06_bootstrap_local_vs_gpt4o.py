...
def load_gpt4o_pool_en_t8() -> tuple[dict[str, bool], dict[str, bool]]:
    ...
    hit = load_search_hit(BSP / 'results' / 'tables' / 'pool_en_faiss_top15.csv', level_key='T_level', level='T8')
    return (top1, hit)
...
def load_gpt4o_hn_t8() -> tuple[dict[str, bool], dict[str, bool]]:
    ...
    hit = load_search_hit(BSP / 'results' / 'tables' / 'hn_faiss_top15.csv', level_key='level', level='T8')
    return (top1, hit)
...
def aligned_vectors(local: dict[str, bool], ref: dict[str, bool]) -> tuple[list[str], np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    p.add_argument('--run-dir', default=str(summer_root() / 'results' / 'runs' / 'p1_full_pool_overnight_20260805'))
    p.add_argument('--out-dir', default=str(summer_root() / 'results' / 'p1_baseline'))
    ...
    for pool in ('pool_en', 'hn'):
        ...
        fields = ['pool', 'T_level', 'query_user_id', 'selected_candidate_user_id', 'selected_candidate_number', 'correct', 'true_in_top15', 'model', 'status']
        with out_jsonl.open('w', encoding='utf-8') as fj, out_csv.open('w', newline='', encoding='utf-8') as fc:
    ...
    specs = [('pool_en', 'top1', 'C1', loc_pe_top1, gpt_pe_top1), ('pool_en', 'hit_at_15', 'C1', loc_pe_hit, gpt_pe_hit), ('hn', 'top1', 'C2', loc_hn_top1, gpt_hn_top1), ('hn', 'hit_at_15', 'C2', loc_hn_hit, gpt_hn_hit)]
    for (pool, metric, claim, local_d, ref_d) in specs:
        ...
        rows.append({'claim': claim, 'pool': pool, 'metric': metric, 'n_local': len(loc_ids), 'n_gpt4o': len(ref_ids), 'n_paired': len(ids), 'local_rate': round(loc_p, 4), 'local_ci_low': round(loc_lo, 4), 'local_ci_high': round(loc_hi, 4), 'local_pct': pct(loc_p), 'gpt4o_rate': round(ref_p, 4), 'gpt4o_ci_low': round(ref_lo, 4), 'gpt4o_ci_high': round(ref_hi, 4), 'gpt4o_pct': pct(ref_p), 'delta_local_minus_gpt4o': round(d_p, 4), 'delta_ci_low': round(d_lo, 4), 'delta_ci_high': round(d_hi, 4), 'delta_pct_pts': round(100.0 * d_p, 2), 'delta_ci_excludes_zero': significant_gap, 'mcnemar_n11': n11, 'mcnemar_n10_local_only': n10, 'mcnemar_n01_gpt4o_only': n01, 'mcnemar_n00': n00, 'mcnemar_p': round(p_mc, 6), 'n_boot': args.n_boot, 'seed': args.seed})
        mcnemar_rows.append({'claim': claim, 'pool': pool, 'metric': metric, 'n_paired': len(ids), 'n11_both': n11, 'n10_local_only': n10, 'n01_gpt4o_only': n01, 'n00_neither': n00, 'p_value': round(p_mc, 6), 'significant_0_05': p_mc < 0.05})
        print(f'{claim} {pool} {metric}: local={pct(loc_p)}% [{pct(loc_lo)}-{pct(loc_hi)}] gpt4o={pct(ref_p)}% [{pct(ref_lo)}-{pct(ref_hi)}] Δ={100 * d_p:+.1f}pp [{100 * d_lo:+.1f},{100 * d_hi:+.1f}] McNemar p={p_mc:.4g} paired={len(ids)}')
    ...
    compact = []
    for r in rows:
        compact.append({'claim': r['claim'], 'pool': r['pool'], 'metric': r['metric'], 'n_paired': r['n_paired'], 'local': f"{r['local_pct']}% [{pct(r['local_ci_low'])}-{pct(r['local_ci_high'])}]", 'gpt4o': f"{r['gpt4o_pct']}% [{pct(r['gpt4o_ci_low'])}-{pct(r['gpt4o_ci_high'])}]", 'delta_pp': f"{r['delta_pct_pts']:+.1f} [{100 * r['delta_ci_low']:+.1f}, {100 * r['delta_ci_high']:+.1f}]", 'mcnemar_p': r['mcnemar_p'], 'delta_ci_excludes_zero': r['delta_ci_excludes_zero']})
